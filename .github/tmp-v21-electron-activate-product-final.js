...
assert.equal(count, 2, 'Product Final must expose exactly two successor admission lists');
text = text.replaceAll(before, after);
assert.equal(text.split("github.event.pull_request.head.ref == 'fix/v21-electron-supported-runtime-p0-production-amendment-1'").length - 1, 2, 'Electron successor must be admitted by both Product Final jobs');
