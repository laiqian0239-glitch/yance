...
function safeLanguage(value) {
  ...
  if (!/^[a-z]{2,12}(?:-[a-z0-9]{2,12})?$/u.test(language) && language !== 'auto' && language !== 'yue') {
...
function createVoiceBrainRuntime(options = {}) {
  ...
  const entrypoint = path.join(appRoot, 'runtime', 'voice-brain', 'cosyvoice', 'yance_cosyvoice_entrypoint.py');
  ...
  function resolveCosyVoiceLayout() {
    ...
    return Object.freeze({
      ...
      python,
      modelDir,
      entrypoint: firstExisting([entrypoint, path.join(runtimeRoot, 'cosyvoice', 'yance_cosyvoice_entrypoint.py')]),
      available: Boolean(python && modelDir && firstExisting([entrypoint, path.join(runtimeRoot, 'cosyvoice', 'yance_cosyvoice_entrypoint.py')]))
    });
  }
  ...
  async function health() {
    ...
    let senseVoice = { authority: 'SenseVoice', available: false, reasonCode: 'SENSEVOICE_STATUS_UNAVAILABLE' };
    try {
      const service = require('../backend/services/transcriptionService');
      senseVoice = service.engineStatus();
    } catch (error) {
      senseVoice = { authority: 'SenseVoice', available: false, reasonCode: clean(error.code) || 'SENSEVOICE_STATUS_UNAVAILABLE' };
    }
    ...
    return Object.freeze({
      ...
      authorities: { asr: 'SenseVoice', tts: 'CosyVoice' },
      senseVoice,
      cosyVoice,
  ...
  async function enrollVoiceProfile(input = {}) {
    ...
    try {
      ...
      const metadata = {
        schemaVersion: 1,
        voiceProfileId,
        label: clean(input.label).slice(0, 160) || 'My voice',
  ...
  function runCosyVoice(payload, timeoutMs = 180000) {
    ...
    if (!layout.available) throw voiceError('COSYVOICE_RUNTIME_MISSING', 'CosyVoice sealed runtime/model is unavailable.', { runtimeRoot });
    return new Promise((resolve, reject) => {
      const child = spawnImpl(layout.python, [layout.entrypoint, '--model-dir', layout.modelDir], {
        windowsHide: true,
        shell: false,
      ...
      const timer = setTimeout(() => {
        child.kill('SIGKILL');
        reject(voiceError('COSYVOICE_TIMEOUT', 'CosyVoice generation timed out.'));
      }, timeoutMs);
      ...
      child.on('close', code => {
        clearTimeout(timer);
        if (code !== 0) return reject(voiceError('COSYVOICE_GENERATION_FAILED', stderr.trim() || `CosyVoice exited with code ${code}.`));
        try { resolve(JSON.parse(stdout.trim())); }
        catch (error) { reject(voiceError('COSYVOICE_RESPONSE_INVALID', 'CosyVoice returned invalid JSON.', { cause: error.message })); }
      });
  ...
  async function generateSpeech(input = {}) {
    ...
    const result = await runCosyVoice({
      ...
      text,
      language,
      outputPath: audioArtifact,
      promptAudio: profile.samplePath,
      promptText: profile.promptText || '',
      sampleLanguage: profile.sampleLanguage || 'auto'
    ...
    return projectVoiceOutput({
      ...
      provenance: Object.freeze({
        authority: 'CosyVoice',
        mode: clean(result.mode) || 'cross-lingual-zero-shot',
