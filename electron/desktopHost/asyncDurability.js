...
// failures and every non-Windows directory failure still fail closed.

const path = require('node:path');
const crypto = require('node:crypto');
const nodeFs = require('node:fs');

// Resolve the promise-capable fs surface from an injected fs module or fake.
// The production default is node:fs (which exposes `fs.promises`). Test fakes
// may expose async methods either directly or under `.promises`.
function promisesOf(fsApi) {
  const candidate = (fsApi && fsApi.promises) || fsApi || nodeFs.promises;
  return candidate;
}
...
function isDirectoryFsyncTolerated(code, platform) {
  ...
  // POSIX tolerates EINVAL/EBADF/ENOTSUP when the filesystem cannot fsync a
  // directory handle; the file rename itself is still durable.
...
// Durable atomic JSON replace. See module docstring for ordering guarantees.
async function atomicWriteJsonAsync(file, value, options = {}) {
  ...
  try {
    ...
    if (dirHandle) {
      ...
      try { await fileSyncAsync(dirHandle, 'directory', platform); } catch (_) { /* tolerated by fileSyncAsync */ }
