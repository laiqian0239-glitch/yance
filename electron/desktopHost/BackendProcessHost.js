...
async function validateBackendLaunchContract(options = {}, fsApi = fs) {
  ...
  if (!await pathExists(options.cwd, fsApi)) throw startupFailure('M1_APP_ROOT_MISSING', 'Backend application root does not exist', { appRoot: options.cwd });
  if (!await pathExists(options.entry, fsApi)) throw startupFailure('M1_BACKEND_ENTRY_MISSING', 'Backend entry file does not exist', { backendEntryPath: options.entry });
  ...
  return Object.freeze({ appRoot: options.cwd, backendEntryPath: options.entry, nodeRuntimeExecutablePath, nodeModulesPath, backendPort, readyTimeoutMs, releaseStartupConfig });
}
...
class BackendProcessHost {
  constructor(options = {}) {
    ...
    if (options.isProcessAlive) registryOptions.isProcessAlive = options.isProcessAlive;
    if (options.isProcessAliveAsync) registryOptions.isProcessAliveAsync = options.isProcessAliveAsync;
    this.ownerRegistry = options.ownerRegistry || new BackendOwnerRegistry(registryOptions);
  ...
  _restoreOwnerRegistry() {
    ...
    if (probe.identityMatch === false) {
      this.ownerRegistryFailure = { reasonCode: 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_MISMATCH_RECOVERY_REQUIRED', message: 'Persisted backend owner PID now resolves to a different process identity; explicit recovery is required', backendPid: Number(record.backendPid || 0), probe, recoveryRequired: true, atUtc: new Date().toISOString() };
    ...
    if (probe.alive === true && probe.identityMatch === null) {
      this.ownerRegistryFailure = { reasonCode: 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_UNVERIFIED_RECOVERY_REQUIRED', message: 'Persisted backend owner is live but its process identity cannot be verified; replacement is fail-closed', backendPid: Number(record.backendPid || 0), probe, recoveryRequired: true, atUtc: new Date().toISOString() };
  ...
  async _refreshRestoredOwnerIdentityForMutation() {
    ...
    const needsAsyncIdentity = currentReasonCode === 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_UNVERIFIED_RECOVERY_REQUIRED' || this.rejectedOwner.pidIdentityMatch == null;
    ...
    const base = { ...this.rejectedOwner, backendPid, pidIdentityMatch: probe.identityMatch, childStillLive: probe.alive === true && probe.identityMatch !== false, restoredFromOwnerRegistry: true };
    if (probe.identityMatch === false) {
      this.ownerRegistryFailure = { reasonCode: 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_MISMATCH_RECOVERY_REQUIRED', message: 'Persisted backend owner PID now resolves to a different process identity; explicit recovery is required', backendPid, probe, recoveryRequired: true, atUtc: new Date().toISOString() };
      this.rejectedOwner = Object.freeze({ ...base, reasonCode: 'WP4_DESKTOP_ORPHAN_OWNER_PID_REUSED', childStillLive: false });
      this._transition(PROCESS_STATES.STOPPED, 'orphan-owner-pid-reused-recovery-required', { backendPid });
      return probe;
    }
    if (probe.alive === true && probe.identityMatch === null) {
      this.ownerRegistryFailure = { reasonCode: 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_UNVERIFIED_RECOVERY_REQUIRED', message: 'Persisted backend owner is live but its process identity cannot be verified; replacement is fail-closed', backendPid, probe, recoveryRequired: true, atUtc: new Date().toISOString() };
      this.rejectedOwner = Object.freeze({ ...base, reasonCode: probe.reasonCode === 'OWNER_LIVENESS_EPERM' ? 'WP4_DESKTOP_ORPHAN_OWNER_LIVENESS_EPERM' : 'WP4_DESKTOP_ORPHAN_OWNER_IDENTITY_UNVERIFIED' });
  ...
  _bindChildLifecycle(child, attempt) {
    ...
    const onMessage = message => {
      ...
      if (message.type === 'backend:startup-failed' && !attempt.error) {
        attempt.error = startupFailure(message.reasonCode || message.code || 'DESKTOP_BACKEND_START_FAILED', message.message || 'Backend startup failed', {
          backendPid: child.pid || 0, phase: String(message.phase || ''), stackHash: String(message.stackHash || ''), causeCodeHash: String(message.causeCodeHash || ''), runtimeSubphase: String(message.runtimeSubphase || '')
        });
    ...
    const onExit = (exitCode, signalCode) => {
      ...
      const admission = attempt.startupAdmission && attempt.ownerClaimRegistered !== true ? attempt.startupAdmission : null;
      if (admission) attempt.startupAdmission = null;

      const recoveryPromise = Promise.resolve().then(async () => {
        let failure = null;
        let credentialRecovery = { recovered: true, notRequired: true };
        if (ownerContext && typeof attempt.handleBackendOwnerExit === 'function') {
          try { credentialRecovery = await attempt.handleBackendOwnerExit(ownerContext); }
          catch (error) { failure = error; }
        }
        if (exitingChildOwnsDurableClaim) {
          try {
            await this.ownerRegistry.markExitedAsync({ exitCode, signalCode, reasonCode: expected ? 'OWNER_EXIT_CONFIRMED' : 'OWNER_EXIT_UNEXPECTED' });
          } catch (registryCause) {
            ...
            if (!failure) failure = registryCause;
          }
        } else {
          this.log('backend-owner-exit-record-skip', { backendPid: Number(child.pid || 0), durableBackendPid: Number(durableOwner?.backendPid || 0), reasonCode: 'EXITING_CHILD_DOES_NOT_OWN_DURABLE_CLAIM' });
        }
        if (admission) {
          try { await admission.release(); }
          catch (registryCause) {
            this.ownerRegistryFailure = { reasonCode: registryCause.reasonCode || 'WP4_DESKTOP_BACKEND_OWNER_CLAIM_LOCK_FAILED', message: registryCause.message, atUtc: new Date().toISOString() };
            if (!failure) failure = registryCause;
          }
        }
      ...
      recoveryPromise.catch(error => this.log('backend-owner-exit-recovery-failed', { backendPid: Number(child.pid || 0), reasonCode: error.reasonCode || error.code || 'WP4_CREDENTIAL_OWNER_EXIT_RECOVERY_FAILED' }));
      this.child = null;
      this.session = null;
    };
  ...
  async _writeStartupFrame(controlPipe, encoded, timeoutMs, signal = null) {
    await new Promise((resolve, reject) => {
      ...
      try { controlPipe.end(encoded, error => error ? onError(error) : finish()); }
  ...
  async _terminateAndWait(child, options = {}) {
    ...
    if (!result.exited) throw startupFailure(killAccepted ? 'DESKTOP_BACKEND_FORCE_EXIT_TIMEOUT' : 'DESKTOP_BACKEND_SIGKILL_FAILED', 'Backend process remained alive after forced termination', { backendPid: child.pid || 0 });
  ...
  _waitForChildMessage(child, attempt, predicate, timeoutMs, timeoutReasonCode) {
    return new Promise((resolve, reject) => {
      ...
      const onExit = (code, signalCode) => finish(startupFailure('DESKTOP_BACKEND_EXIT_DURING_START', 'Backend exited before startup handshake completed', { backendPid: child.pid || 0, exitCode: code, signalCode: signalCode || null }));
      ...
      const onAbort = () => {
        ...
        finish(reason instanceof Error ? reason : startupFailure('DESKTOP_BACKEND_START_CANCELLED', 'Backend startup handshake was cancelled', { backendPid: child.pid || 0 }));
      };
  ...
  async _startUnlocked(options = {}) {
    ...
    if ((this.autoRecoverRejectedOwner || options.autoRecoverRejectedOwner === true) && (this.rejectedOwner || this.ownerRegistryFailure)) await this._recoverRejectedOwnerForStartUnlocked(options);
    ...
    this._transition(PROCESS_STATES.STARTING, 'start-requested', {
      backendEntryPath: options.entry || '', appRoot: options.cwd || '', nodeRuntimeExecutablePath: options.nodeRuntimeExecutablePath || options.execPath || '', nodeModulesPath: options.nodeModulesPath || options.env?.NODE_PATH || ''
    });
    ...
    const defaultOwnerExitRecovery = options.credentialVaultHost && typeof options.credentialVaultHost.handleBackendOwnerExit === 'function' ? ownerContext => options.credentialVaultHost.handleBackendOwnerExit(ownerContext) : null;
    const attempt = {
      startupAttemptId, startupNonce, backendSessionId, fd6PipeInstanceId, cancelled: false, error: null, exit: null, listeners: null, ownerContext: null,
      startupAdmission: null, ownerClaimRegistered: false, abortController: new AbortController(), handleBackendOwnerExit: options.handleBackendOwnerExit || defaultOwnerExitRecovery
    };
    ...
    try {
      ...
      this.log('backend-process-launch-contract', {
        nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath, backendEntryPath: launchContract.backendEntryPath, appRoot: launchContract.appRoot,
        nodeModulesPath: launchContract.nodeModulesPath || '', dataDir: sanitizedEnv.YANCE_DATA_DIR || '', port: sanitizedEnv.YANCE_PORT || '',
      ...
      const custodyConnectionPromise = new Promise((resolve, reject) => { resolveCustodyConnection = resolve; rejectCustodyConnection = reject; });
      custodyConnectionPromise.catch(() => {});
      let credentialFrame = null;
      if (Boolean(options.credentialVaultHost)) {
        const custodyServer = this.createCredentialCustodyServer();
        if (!custodyServer || typeof custodyServer.listen !== 'function' || typeof custodyServer.on !== 'function') throw startupFailure('DESKTOP_CREDENTIAL_CUSTODY_PIPE_UNAVAILABLE', 'Credential custody server factory returned an invalid server');
        ...
        custodyServer.on('connection', socket => {
          if (this.credentialCustodyHost) { try { socket.destroy(); } catch (_) {} return; }
          try {
            this.credentialCustodyHost = new CredentialCustodyHost({
              stream: socket,
              vaultHost: options.credentialVaultHost,
              context: { backendPid: child.pid, startupNonce, backendSessionId, fd6PipeInstanceId, manifestSha256: options.releaseStartupConfig.manifestSha256, vaultEpoch: credentialFrame.vaultEpoch, generation: credentialFrame.generation, hydrationGeneration: credentialFrame.generation }
            });
            resolveCustodyConnection(this.credentialCustodyHost);
          } catch (error) { try { socket.destroy(); } catch (_) {} rejectCustodyConnection(error); }
        });
        await new Promise((resolve, reject) => {
      ...
      const runtimeProbe = await this.probeNodeRuntime(launchContract.nodeRuntimeExecutablePath, { cwd: options.cwd, env: sanitizedEnv, timeoutMs: options.nodeRuntimeProbeTimeoutMs || 5000 });
      this.log('backend-node-runtime-preflight-pass', { nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath, nodeRuntimeVersion: runtimeProbe.version });
      attempt.startupAdmission = await this.ownerRegistry.acquireStartupAdmission();
      this.log('backend-process-fork-dispatch', { nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath, backendEntryPath: options.entry, appRoot: options.cwd, inheritedElectronExecArgvCount: Array.isArray(process.execArgv) ? process.execArgv.length : 0, backendExecArgvCount: 0 });
      child = this.fork(options.entry, [], { execPath: launchContract.nodeRuntimeExecutablePath, execArgv: [], cwd: options.cwd, env: sanitizedEnv, stdio, windowsHide: options.windowsHide !== false, windowsVerbatimArguments: false, serialization: 'json' });
      this.log('backend-process-fork-returned', { backendPid: Number(child?.pid || 0), nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath, backendEntryPath: options.entry });
      ...
      if (typeof options.createCredentialSnapshot === 'function') {
        prepared = await options.createCredentialSnapshot({ startupNonce, oneTimeToken: credentialOneTimeToken, backendPid: child.pid, manifestSha256: options.releaseStartupConfig.manifestSha256, backendSessionId, fd6PipeInstanceId });
      } else {
      ...
      credentialFrame = prepared?.frame || prepared;
      const resetAuthorization = prepared?.resetAuthorization || null;
      const ownerContext = Object.freeze(prepared?.ownerSession || { backendPid: child.pid, startupNonce, backendSessionId, manifestSha256: options.releaseStartupConfig.manifestSha256, vaultEpoch: credentialFrame.vaultEpoch, hydrationGeneration: credentialFrame.generation, fd6PipeInstanceId });
      ...
      if (requireHandshake && !custodyVaultAvailable) throw startupFailure('CREDENTIAL_VAULT_UNAVAILABLE', 'CredentialVaultHost is required for the custody channel');
      if (requireHandshake && !this.credentialCustodyServer) throw startupFailure('DESKTOP_CREDENTIAL_CUSTODY_PIPE_UNAVAILABLE', 'Dedicated credential custody pipe is unavailable');
      ...
      const encoded = this.encodeStartupFrame({
        protocolVersion: STARTUP_PROTOCOL_VERSION, startupFrameProtocolVersion: STARTUP_FRAME_PROTOCOL_VERSION, m1StartupContractVersion: M1_STARTUP_CONTRACT_VERSION,
        readyProtocolVersion: READY_PROTOCOL_VERSION, startupAttemptId, startupNonce, apiSessionToken, backendPid: child.pid, resourcesPath: releaseContract.resourcesPath,
        expectedBuildId: releaseContract.expectedBuildId, manifestSha256: releaseContract.manifestSha256,
        ...
        appRoot: launchContract.appRoot, backendEntryPath: launchContract.backendEntryPath, nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath,
        nodeModulesPath: launchContract.nodeModulesPath || '', runtimeMode: RUNTIME_MODE_DESKTOP_HOSTED, backendSessionId, fd6PipeInstanceId, backendPort,
        apiBaseUrl: `http://127.0.0.1:${backendPort}`, readyTimeoutMs, launchTimeoutMs: Number(options.launchTimeoutMs || readyTimeoutMs), stopTimeoutMs: Number(options.stopTimeoutMs || options.timeoutMs || 7000),
        dataDir: String(sanitizedEnv.YANCE_DATA_DIR || ''), logPath: backendLogPath, logRoot, desktopLogPath, backendLogPath,
        credentialProtocolVersion: CREDENTIAL_PROTOCOL_VERSION, credentialOneTimeToken, credentialVaultEpoch: credentialFrame.vaultEpoch,
        credentialGeneration: credentialFrame.generation, credentialBackendSessionId: backendSessionId, credentialFd6PipeInstanceId: fd6PipeInstanceId,
        credentialAuthorityEventId: credentialFrame.authorityEventId, credentialAuthorityHeadDigest: credentialFrame.authorityHeadDigest,
        credentialVaultReferenceCount: credentialFrame.vaultReferenceCount, credentialDecryptedEntryCount: credentialFrame.decryptedEntryCount,
        credentialFrameEntryCount: credentialFrame.frameEntryCount, credentialResetAuthorization: resetAuthorization
      });
      ...
      if (shouldDeliverCredentialSnapshot) {
        ...
        await this.credentialIpcHost.sendSnapshot(credentialFrame, { timeoutMs: options.credentialWriteTimeoutMs, signal: attempt.abortController.signal });
      ...
      if (requireHandshake) {
        ...
        const expectedCredentialMetadata = {
          pid: child.pid, startupNonce, vaultEpoch: credentialFrame.vaultEpoch, generation: credentialFrame.generation, authorityEventId: credentialFrame.authorityEventId,
          vaultReferenceCount: credentialFrame.vaultReferenceCount, decryptedEntryCount: credentialFrame.decryptedEntryCount, frameEntryCount: credentialFrame.frameEntryCount,
          entryCount: credentialFrame.payload.entries.length, payloadBytes: credentialFrame.payloadBytes, restoredReferenceCount: credentialFrame.payload.entries.length
        };
        ...
        if (options.readyHealthCheckPath || options.readyHealthCheck === true) {
          const readyHealth = await probeBackendHttpReady({ port: readiness.port, token: apiSessionToken, path: options.readyHealthCheckPath || '/api/health', timeoutMs: options.readyHealthCheckTimeoutMs || Math.min(5000, readyTimeoutMs), retries: options.readyHealthCheckRetries ?? 20, retryDelayMs: options.readyHealthCheckRetryDelayMs || 100 });
        ...
        } finally { if (custodyTimer) clearTimeout(custodyTimer); }
        if (!this.credentialCustodyHost?.snapshot?.().dedicatedPipeActive) throw startupFailure('DESKTOP_CREDENTIAL_CUSTODY_PIPE_UNAVAILABLE', 'Dedicated credential custody pipe is not active at the READY boundary', { backendPid: child.pid });
      }
      ...
      this.session = Object.freeze({
        startupAttemptId, startupNonce, backendSessionId, fd6PipeInstanceId, apiSessionToken, backendPid: child.pid,
        readyProtocolVersion: readiness?.readyProtocolVersion || READY_PROTOCOL_VERSION,
        runtimeContract: Object.freeze({ startupAttemptId, m1StartupContractVersion: M1_STARTUP_CONTRACT_VERSION, startupFrameProtocolVersion: STARTUP_FRAME_PROTOCOL_VERSION, readyProtocolVersion: READY_PROTOCOL_VERSION, runtimeMode: RUNTIME_MODE_DESKTOP_HOSTED, appRoot: launchContract.appRoot, backendEntryPath: launchContract.backendEntryPath, nodeRuntimeExecutablePath: launchContract.nodeRuntimeExecutablePath, nodeModulesPath: launchContract.nodeModulesPath || '', backendPort, apiBaseUrl: `http://127.0.0.1:${backendPort}`, readyTimeoutMs, releaseManifestPath: releaseContract.releaseManifestPath || releaseContract.manifestPath || path.join(releaseContract.resourcesPath, 'release-manifest.json'), releaseManifestSha256Path: releaseContract.releaseManifestSha256Path || releaseContract.detachedHashPath || path.join(releaseContract.resourcesPath, 'release-manifest.sha256'), logRoot, desktopLogPath, backendLogPath }),
        credentialFrameDelivered: shouldDeliverCredentialSnapshot, credentialHydrated: Boolean(hydration), credentialVaultEpoch: credentialFrame.vaultEpoch,
        credentialGeneration: credentialFrame.generation, credentialAuthorityEventId: credentialFrame.authorityEventId, credentialAuthorityHeadDigest: credentialFrame.authorityHeadDigest,
        credentialVaultReferenceCount: credentialFrame.vaultReferenceCount, credentialDecryptedEntryCount: credentialFrame.decryptedEntryCount, credentialFrameEntryCount: credentialFrame.frameEntryCount,
        ownerContext, readyCredentialMetadata: readiness?.credentialMetadata ? Object.freeze({ ...readiness.credentialMetadata }) : null
      });
      ...
      error.backendStartDiagnostics = { backendPid: child?.pid || 0, state: this.state, childExited: processExited(child), childExitCode: child?.exitCode ?? null, childSignalCode: child?.signalCode || null };
      ...
      if (liveOwnedChild) {
        ...
        if (childReceivedCredentialAuthority) {
          try { error.rejectedOwnerContainment = this.containRejectedOwner({ reasonCode, ownerSession: attempt.ownerContext || null, persistOwnerRecord: false }); }
          catch (containmentError) { error.containmentReasonCode = containmentError.reasonCode || containmentError.code || 'WP4_DESKTOP_REJECTED_OWNER_CONTAINMENT_FAILED'; }
        } else {
          error.rejectedOwnerContainmentSkipped = true;
          error.rejectedOwnerContainmentSkippedReason = 'CHILD_FAILED_BEFORE_OWNER_CLAIM';
        }
        try { error.startFailureTermination = await this._terminateAndWait(child, { gracefulMs: 100, forceMs: Number(options.forceExitTimeoutMs || 3000) }); }
        catch (cleanupError) { error.cleanupReasonCode = cleanupError.reasonCode || cleanupError.code || 'DESKTOP_BACKEND_EXIT_NOT_CONFIRMED'; }
      }
      if (attempt.startupAdmission) {
        ...
        if (durableClaimNowBlocksReplacement || preClaimChildContained) {
          ...
          catch (releaseError) { error.ownerClaimReleaseFailure = { reasonCode: releaseError.reasonCode || releaseError.code || 'WP4_DESKTOP_BACKEND_OWNER_CLAIM_LOCK_FAILED', message: releaseError.message }; }
          attempt.startupAdmission = null;
        } else {
      ...
      if (child && processExited(child)) {
        ...
        catch (recoveryError) { error.ownerRecoveryReasonCode = recoveryError.reasonCode || recoveryError.code || 'WP4_CREDENTIAL_OWNER_EXIT_RECOVERY_FAILED'; }
        if (recovery?.recovered === true && this.rejectedOwner) {
          try { await this.clearRejectedOwner({ force: false, observedExitedChild: child }); }
          catch (recoveryError) { error.ownerRecoveryReasonCode = recoveryError.reasonCode || recoveryError.code || 'WP4_CREDENTIAL_OWNER_EXIT_RECOVERY_FAILED'; }
        } else if (recovery?.recovered !== true && !error.ownerRecoveryReasonCode) error.ownerRecoveryReasonCode = 'WP4_CREDENTIAL_OWNER_EXIT_RECOVERY_PENDING';
      }
  ...
  async _recoverRejectedOwnerForStartUnlocked(options = {}) {
    await this._refreshRestoredOwnerIdentityForMutation();
    const mismatchFailure = this.ownerRegistryFailure?.reasonCode === 'WP4_DESKTOP_BACKEND_OWNER_IDENTITY_MISMATCH_RECOVERY_REQUIRED' && this.rejectedOwner?.pidIdentityMatch === false;
    if (mismatchFailure) {
      ...
      await this.clearRejectedOwner({ force: true });
      return { recovered: true, backendPid, pidReused: true, forced: false };
    }
    if (this.ownerRegistryFailure) throw startupFailure(this.ownerRegistryFailure.reasonCode || 'WP4_DESKTOP_BACKEND_OWNER_REGISTRY_INVALID', 'Automatic backend owner recovery is unsafe because the durable owner identity cannot be trusted', { registryFailure: this.ownerRegistryFailure, automaticRecoveryAttempted: true });
    ...
    const stopped = await this._terminateOrphanOwner({ gracefulMs: options.ownerRecoveryGracefulMs || options.gracefulMs || 1500, forceMs: options.ownerRecoveryForceMs || options.forceMs || 1500 });
    if (stopped?.stopped !== true || stopped?.exitConfirmed !== true) throw startupFailure(stopped?.reasonCode || 'WP4_DESKTOP_REJECTED_OWNER_AUTOMATIC_RECOVERY_FAILED', 'The previous Yance backend owner could not be safely recovered automatically', { backendPid, stopped, automaticRecoveryAttempted: true });
  ...
  recoverRejectedOwnerForStart(options = {}) { return this._enqueue(() => this._recoverRejectedOwnerForStartUnlocked(options)); }

  async _terminateOrphanOwner(options = {}) {
    await this._refreshRestoredOwnerIdentityForMutation();
    if (this.ownerRegistryFailure) return { stopped: false, exitConfirmed: false, backendPid: Number(this.rejectedOwner?.backendPid || 0), reasonCode: this.ownerRegistryFailure.reasonCode || 'WP4_DESKTOP_BACKEND_OWNER_REGISTRY_INVALID', registryFailure: { ...this.ownerRegistryFailure }, state: this.state };
    ...
    if (probe.identityMatch !== true) return { stopped: false, exitConfirmed: false, backendPid, reasonCode: probe.reasonCode || 'WP4_DESKTOP_ORPHAN_OWNER_IDENTITY_UNVERIFIED', state: this.state };
    ...
    const waitDead = async timeoutMs => {
      ...
      while (Date.now() < deadline) {
        if (!(await this.ownerRegistry.probeAsync(record)).alive) return true;
        await new Promise(resolve => setTimeout(resolve, 25));
      }
      return !(await this.ownerRegistry.probeAsync(record)).alive;
    };
    let termAccepted = false;
    try { this.killProcess(backendPid, 'SIGTERM'); termAccepted = true; }
    catch (cause) {
    ...
    if (!exited) {
      forced = true;
      try { this.killProcess(backendPid, 'SIGKILL'); }
      catch (cause) { if (cause?.code !== 'ESRCH') return { stopped: false, exitConfirmed: false, backendPid, reasonCode: cause?.code === 'EPERM' ? 'WP4_DESKTOP_ORPHAN_OWNER_TERMINATION_EPERM' : 'WP4_DESKTOP_ORPHAN_OWNER_SIGKILL_FAILED', state: this.state }; }
  ...
  async _stopUnlocked(options = {}) {
    ...
    if (!child) {
      if (this.rejectedOwner || this.ownerRegistry.snapshot()?.ownershipActive === true) return this._terminateOrphanOwner(options);
      this.session = null;
      this.startAttempt = null;
    ...
    try {
      ...
      if (recovery?.recovered !== true) throw startupFailure('WP4_CREDENTIAL_OWNER_EXIT_RECOVERY_PENDING', 'Backend exit occurred but credential owner recovery is not complete', { recovery, backendPid: child.pid || 0 });
      if (this.child === child) {
        this.child = null;
        this.session = null;
        this.credentialIpcHost.close();
  ...
  async acceptBackendOwner(context = {}) {
    if (this.rejectedOwner || this.ownerRegistryFailure) throw startupFailure('WP4_DESKTOP_BACKEND_OWNER_ACCEPTANCE_BLOCKED', 'Rejected or registry-invalid backend owner cannot be accepted', { rejectedOwner: this.rejectedOwner, registryFailure: this.ownerRegistryFailure });
    const backendPid = Number(context.backendPid || this.child?.pid || this.session?.backendPid || 0);
    if (!this.child || processExited(this.child) || !this.session || backendPid !== Number(this.child.pid || 0)) throw startupFailure('WP4_DESKTOP_BACKEND_OWNER_ACCEPTANCE_STALE', 'Backend owner acceptance no longer matches the live child', { backendPid });
    try {
  ...
  containRejectedOwner(context = {}) {
    ...
    const exactOwnedChildLive = Boolean(child && Number(child.pid || 0) === backendPid && !processExited(child));
    const durableOwnerPotentiallyLive = Boolean(!child && registry?.ownershipActive === true && Number(registry.backendPid || 0) === backendPid);
    const childStillLive = exactOwnedChildLive || durableOwnerPotentiallyLive;
    const pidIdentityMatch = exactOwnedChildLive ? true : (this.rejectedOwner?.pidIdentityMatch ?? null);
    const base = {
      backendPid,
    ...
    this.rejectedOwner = Object.freeze({ ...base, apiAuthorityRevoked: !this.session?.apiSessionToken, genericPipeClosed, fd6Closed, ownerRecordDurable: false, closeFailures });
    this._recordFailure(this.rejectedOwner.reasonCode, child);
    if (childStillLive && [PROCESS_STATES.RUNNING, PROCESS_STATES.STARTING].includes(this.state)) this._transition(PROCESS_STATES.STOPPING, 'rejected-owner-contained', { backendPid });
  ...
  async persistRejectedOwnerMarker(context = {}) {
    if (!this.rejectedOwner) throw startupFailure('WP4_DESKTOP_REJECTED_OWNER_MARKER_MISSING', 'Rejected owner cannot be persisted before in-memory containment is established');
    const closeFailures = [...(this.rejectedOwner.closeFailures || [])];
    let ownerRecordDurable = false;
    if (this.ownerRegistryFailure) closeFailures.push({ channel: 'OWNER_REGISTRY', message: this.ownerRegistryFailure.message || this.ownerRegistryFailure.reasonCode });
    else {
      try {
        await this.ownerRegistry.markRejectedAsync({ reasonCode: String(context.reasonCode || this.rejectedOwner.reasonCode || 'WP4_DESKTOP_CREDENTIAL_REJECTED_OWNER'), ownerSession: context.ownerSession || this.session?.ownerContext || this.ownerRegistry.snapshot()?.ownerSession || null });
        this.orphanOwnerRecord = this.ownerRegistry.snapshot();
        this.ownerRegistryFailure = null;
    ...
    this.rejectedOwner = Object.freeze({ ...this.rejectedOwner, ownerRecordDurable, closeFailures, ownerRecordPersistedAtUtc: ownerRecordDurable ? new Date().toISOString() : '' });
    return this.rejectedOwner;
  }

  async clearRejectedOwner(options = {}) {
    if (!this.rejectedOwner) return false;
    if (this.ownerRegistryFailure && options.force !== true) throw startupFailure('WP4_DESKTOP_BACKEND_OWNER_REGISTRY_RECOVERY_BLOCKED', 'Rejected owner marker cannot be cleared while the durable owner registry is invalid or unavailable', { registryFailure: this.ownerRegistryFailure });
    const child = this.child;
    const record = this.ownerRegistry.snapshot();
    ...
    const observedExitMatchesOwner = Boolean(
      observedExitedChild && this.ownerExitRecoveryByChild.has(observedExitedChild) && observedExitedChild.__desktopHostExited === true && processExited(observedExitedChild)
      && Number(observedExitedChild.pid || 0) === backendPid && Number(this.lastExit?.backendPid || 0) === backendPid && record?.state === 'EXITED' && record?.ownershipActive === false
    );
    const probe = observedExitMatchesOwner
      ? { alive: false, identityMatch: true, reasonCode: 'OWNER_EXIT_EVENT_CONFIRMED', backendPid }
      : await this.ownerRegistry.probeAsync(record || { backendPid });
    if (options.force !== true && ((child && !processExited(child)) || (probe.alive === true && probe.identityMatch !== false))) throw startupFailure('WP4_DESKTOP_REJECTED_OWNER_STILL_LIVE', 'Rejected owner marker cannot be cleared while its child remains live', { backendPid, probe });
    try {
  ...
  isRejectedOwnerLive() {
    ...
    const record = this.ownerRegistry.snapshot();
    return Boolean(record?.ownershipActive === true || this.rejectedOwner.childStillLive === true);
  }

  async isRejectedOwnerLiveAsync() {
    if (!this.rejectedOwner) return false;
    if (this.child && !processExited(this.child)) return true;
    const record = this.ownerRegistry.snapshot();
    if (!record) return this.rejectedOwner.childStillLive === true;
    const probe = await this.ownerRegistry.probeAsync(record);
  ...
  waitForOwnerExitRecovery(child = null) {
    ...
    if (!target) {
      if (this.rejectedOwner || this.ownerRegistry.snapshot()?.ownershipActive === true) return Promise.resolve({ recovered: false, ownerRecoveryRequired: true, pendingExitEvent: false });
    ...
    if (processExited(target)) return Promise.resolve({ recovered: false, pendingExitEvent: false, pendingOwnerRecovery: true, backendPid: target.pid || 0 });
  ...
  hasOwnership() {
    ...
    const durableOwnership = this.ownerRegistry.snapshot()?.ownershipActive === true;
    return Boolean(liveChild || this.startAttempt || durableOwnership || [PROCESS_STATES.STARTING, PROCESS_STATES.RUNNING, PROCESS_STATES.STOPPING].includes(this.state));
  }
  ...
  snapshot() {
    ...
    const ownerTrusted = Boolean(!this.rejectedOwner && !this.ownerRegistryFailure && (!ownershipClaimActive || (ownerRegistry?.state === 'RUNNING' && ownerRegistry?.ownershipActive === true && ownerRegistry?.trusted === true)));
    return Object.freeze({
      state: this.state,
      running: Boolean(this.child && !processExited(this.child) && this.state === PROCESS_STATES.RUNNING),
      backendPid: this.child?.pid || Number((this.rejectedOwner?.childStillLive ? this.rejectedOwner.backendPid : 0) || (ownerRegistry?.ownershipActive ? ownerRegistry?.backendPid : 0) || 0),
      startupNonce: this.session?.startupNonce || this.rejectedOwner?.startupNonce || ownerRegistry?.startupNonce || null,
