...
const DEFAULT_GENERIC_AUTHORIZATION_PATH_CACHE = new Map();
const DEFAULT_AUTHORIZATION_JSON_CACHE = new Map();

function trustedGitObjectCacheRoot(options = {}) {
  return path.resolve(options.trustedPolicyRoot || TRUSTED_POLICY_ROOT);
}

function defaultListGenericAuthorizationPaths(trustedMainHead, options = {}) {
  const cacheKey = `${trustedGitObjectCacheRoot(options)}\u0000${trustedMainHead}`;
  const cached = DEFAULT_GENERIC_AUTHORIZATION_PATH_CACHE.get(cacheKey);
  ...
  try {
    ...
    const filtered = Array.isArray(values)
      ? values.filter(isGenericDelegatedGovernanceAuthorizationPath)
      : [];

    DEFAULT_GENERIC_AUTHORIZATION_PATH_CACHE.set(
      cacheKey,
...
function defaultAuthorizationAtCommit(commit, repositoryPath, options = {}) {
  const cacheKey = `${trustedGitObjectCacheRoot(options)}\u0000${commit}\u0000${repositoryPath}`;
  let serialized = DEFAULT_AUTHORIZATION_JSON_CACHE.get(cacheKey);

  if (serialized === undefined) {
    try {
      serialized = trustedGit(['show', `${commit}:${repositoryPath}`], options);
    } catch (_) {
      return null;
    }

    DEFAULT_AUTHORIZATION_JSON_CACHE.set(cacheKey, serialized);
  }
...
function evaluateTrustedDelegatedGovernanceBranch(options = {}) {
  ...
  while (topologyExpanded) {
    topologyExpanded = false;

    for (const [sourcePath, declaration] of declarations) {
      const targetPath = declaration.authorizationPath;

      if (!topologyPaths.has(sourcePath)
        && !topologyPaths.has(targetPath)) continue;

      for (const connectedPath of [sourcePath, targetPath]) {
        if (!authorizations.has(connectedPath)
          || topologyPaths.has(connectedPath)) continue;

        topologyPaths.add(connectedPath);
