...
test('Daily Conversation Review rejects missing or invalid explicit day/timezone identity', () => {
  ...
  assert.throws(
    () => workspaceService.dailyReview('contact-a', { ...options, timeZone: 'UTC' }),
    error => error?.code === 'DAILY_REVIEW_LOCAL_DATE_INVALID'
  );
  assert.throws(
    () => workspaceService.dailyReview('contact-a', { ...options, localDate: '2026-02-31', timeZone: 'UTC' }),
    error => error?.code === 'DAILY_REVIEW_LOCAL_DATE_INVALID'
  );
  assert.throws(
    () => workspaceService.dailyReview('contact-a', { ...options, localDate: '2026-09-05', timeZone: 'Not/A_Real_Zone' }),
    error => error?.code === 'DAILY_REVIEW_TIME_ZONE_INVALID'
  );
});
