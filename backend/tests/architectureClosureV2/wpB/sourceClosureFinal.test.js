...
test('M3-SC-002 base plus authorized extension inventory remains exact and unique', () => {
  ...
  assert.equal(
    inventory.entries.some(entry => entry.id === 'WPB-SYNC-CHECKPOINT-SERVICE'),
    false,
    'syncCheckpointService is a compatibility mutation facade, not a separate recovery source authority'
  );
});
