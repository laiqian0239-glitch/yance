...
test('completed advisories-bulk network timeout retries exactly once and preserves raw transport evidence on exhaustion', () => {
  ...
  assert.throws(
    ...
    error => {
      assert.equal(error.code, 'WP_B_UPSTREAM_NPM_AUDIT_ENDPOINT_NETWORK_TIMEOUT');
...
test('completed npm audit results outside the exact transient signature never retry', () => {
  const exactMessage = 'network timeout at: https://registry.npmjs.org/-/npm/v1/security/advisories/bulk';
  const cases = [
    Object.freeze({
      status: 1,
      stdout: JSON.stringify({
        message: 'network timeout at: https://registry.npmjs.org/-/npm/v1/security/advisories/other',
        error: { summary: '', detail: '' }
      }),
      stderr: ''
    }),
  ...
  for (const result of cases) {
    ...
    if (result.stdout === '{"message":') {
      assert.throws(
        () => packageVerifier.parseJsonOutput(returned, 'npm audit'),
