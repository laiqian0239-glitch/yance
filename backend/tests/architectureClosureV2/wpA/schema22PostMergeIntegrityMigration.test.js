...
test('Schema 22 upgrades a Schema 21 database without inventing historical event content', () => withRawDatabase(db => {
  ...
  assert.ok(columnNames(db, 'authority_command_receipts').includes('content_hash_version'));

  const receipt = db.prepare(`SELECT command_content_sha256,event_content_sha256,content_hash_version
  ...
  assert.equal(receipt.content_hash_version, 0);
...
test('Schema 22 enforces one payload identifier and append-only immutable authorities', () => withRawDatabase(db => {
  ...
  const receiptTriggers = triggerNames(db, 'authority_command_receipts');
  assert.ok(receiptTriggers.includes(RECEIPT_HASH_INSERT_TRIGGER));

  const insertReceipt = db.prepare(`INSERT INTO authority_command_receipts(
  ...
  ) VALUES(?,?,?,?,?,?,'COMMITTED','','',0,1,1,'{}','2026-08-03T00:00:00.000Z')`);
  assert.throws(
    () => insertReceipt.run('invalid-command','IntegrityAuthority','invalid-key','','',0),
    /requires verified content hashes/u
  );
  insertReceipt.run(
    'verified-command',
    'IntegrityAuthority',
  ...
  const verified = db.prepare(`SELECT content_hash_version FROM authority_command_receipts
    WHERE command_id='verified-command'`).get();
  assert.equal(verified.content_hash_version, 1);
