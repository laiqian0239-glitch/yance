...
test('21 Telegram expression reference is opaque', () => { const row = references.create({ accountId: 'a1', kind: 'sticker', document: { id: 7 } }); assert.match(row.reference, /^tgx_[A-Za-z0-9_-]+$/u); assert.equal(row.reference.includes('a1'), false); assert.equal(row.reference.includes('sticker'), false); });
