...
test('round12 migration creates platform, identity, event, strategy and learning authorities', () => {
  withStore(store => {
    ...
    assert.equal(store.getMeta('schema_version', 0), store.supportedSchemaVersion());
...
test('round12 migration is idempotent and older consistency checks do not downgrade schemaVersion', () => {
  ...
  try {
    ...
    assert.equal(first.getMeta('schemaVersion', 0), first.supportedSchemaVersion());
    assert.equal(first.getMeta('schema_version', 0), first.supportedSchemaVersion());
    first.close(); first = null;
    second = new R32SqliteStore({ dbPath });
    assert.equal(second.getMeta('schemaVersion', 0), second.supportedSchemaVersion());
    assert.equal(second.getMeta('schema_version', 0), second.supportedSchemaVersion());
...
test('schema governance reads both historical keys and refuses either key ahead of the binary', () => {
  for (const aheadKey of ['schema_version', 'schemaVersion']) {
    ...
    try {
      store = new R32SqliteStore({ dbPath });
      const aheadVersion = store.supportedSchemaVersion() + 1;
      store.setMeta(aheadKey, aheadVersion);
      store.close(); store = null;
      assert.throws(
        () => { store = new R32SqliteStore({ dbPath }); },
        error => error?.reasonCode === 'SCHEMA_VERSION_AHEAD' && error?.databaseVersion === aheadVersion,
        aheadKey
      );
    } finally {
      try { store?.close(); } catch (_) {}
