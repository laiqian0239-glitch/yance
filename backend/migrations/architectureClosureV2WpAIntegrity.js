...
function ensureConsistency(db) {
  ...
  const invalidHash = db.prepare(`SELECT command_id FROM authority_command_receipts
    WHERE command_content_sha256<>'' AND command_content_sha256 NOT GLOB '[0-9a-f]*'
       OR event_content_sha256<>'' AND event_content_sha256 NOT GLOB '[0-9a-f]*'
    LIMIT 1`).get();
  if (invalidHash) {
    const error = new Error('Schema 22 receipt content hash contains invalid characters');
    error.code = 'ACV2_WP_A_RECEIPT_CONTENT_HASH_INVALID';
    error.commandId = String(invalidHash.command_id || '');
    throw error;
  }
}
