...
function assertExactPlainObject(input, fields, name) {
  if (!isPlainObject(input)) throw coordinatorError('AUTHORITY_TRANSACTION_INPUT_INVALID', `${name} must be a plain object`, { name });
...
function requiredString(value, field, maximum = 1024) {
  ...
  if (result.length > maximum || /[\u0000-\u001f\u007f]/u.test(result)) {
    throw coordinatorError('AUTHORITY_TRANSACTION_FIELD_INVALID', `${field} is invalid`, { field, length: result.length, maximum });
  }
  return result;
}
...
function normalizeEvent(input) {
  ...
  for (const field of ['eventId', 'eventType', 'schemaVersion', 'payloadClassification', 'occurredAt', 'payload']) {
    if (!Object.prototype.hasOwnProperty.call(descriptors, field)) {
      throw coordinatorError('AUTHORITY_TRANSACTION_FIELD_REQUIRED', `event.${field} is required`, { field: `event.${field}` });
    }
  }
  ...
  if (!Number.isSafeInteger(schemaVersion) || schemaVersion < 1) {
    throw coordinatorError('AUTHORITY_TRANSACTION_EVENT_SCHEMA_INVALID', 'event.schemaVersion must be a positive safe integer');
  }
  const payloadClassification = requiredString(descriptors.payloadClassification.value, 'event.payloadClassification', 64);
  if (!CLASSIFICATION_VALUES.has(payloadClassification)) {
    throw coordinatorError('AUTHORITY_TRANSACTION_EVENT_CLASSIFICATION_INVALID', 'event.payloadClassification is not registered', { payloadClassification });
  }
  let payloadCanonical;
  try { payloadCanonical = canonicalSerialize(descriptors.payload.value); }
  catch (error) {
    throw coordinatorError('AUTHORITY_TRANSACTION_EVENT_PAYLOAD_INVALID', 'event.payload is not canonical plain data', {
  ...
  if (!Number.isSafeInteger(generation) || generation < 0) {
    throw coordinatorError('AUTHORITY_TRANSACTION_EVENT_GENERATION_INVALID', 'event.generation must be a non-negative safe integer');
  }
  return Object.freeze({
    ...
    generation,
    redactionVersion: optionalString(descriptors.redactionVersion?.value, 'event.redactionVersion', 128) || 'classification-v1',
    retentionClass: optionalString(descriptors.retentionClass?.value, 'event.retentionClass', 128) || 'ACTIVE_REPLAY',
    ledgerSegmentId: optionalString(descriptors.ledgerSegmentId?.value, 'event.ledgerSegmentId', 512) || 'segment-active-v1'
  });
}
...
function normalizeProjector(input) {
  ...
  for (const field of PROJECTOR_FIELDS) {
    if (!Object.prototype.hasOwnProperty.call(descriptors, field)) {
      throw coordinatorError('AUTHORITY_TRANSACTION_FIELD_REQUIRED', `projector.${field} is required`, { field: `projector.${field}` });
    }
  }
  ...
  if (typeof apply !== 'function') throw coordinatorError('AUTHORITY_TRANSACTION_PROJECTOR_INVALID', 'projector.apply must be a function');
...
function receiptFromRow(row, replayed) {
  ...
  return Object.freeze({
    ...
    result: Object.freeze(resultDocument.result || {})
  });
}
...
class AuthorityTransactionCoordinator {
  constructor(options = {}) {
    ...
    this.onTransactionTelemetry = typeof options.onTransactionTelemetry === 'function' ? options.onTransactionTelemetry : (() => {});
  }
  ...
  assertIdempotency(existing, command) {
    ...
    if (String(document.commandContentSha256 || '') !== command.contentSha256) {
      throw coordinatorError('AUTHORITY_COMMAND_IDEMPOTENCY_CONFLICT', 'The idempotency key already belongs to different command content', {
  ...
  execute(input = {}) {
    ...
    assertAuthorityCommandEnvelope(command);
    ...
    try {
      committed = this.store.transaction(() => runWithAuthorityWriteTransaction({
        ...
        if (Number(insertHeader.changes || 0) !== 1) {
          ...
          throw coordinatorError('AUTHORITY_AGGREGATE_VERSION_CONFLICT', 'Aggregate expected version did not match the current ledger version', {
        ...
        if (projectionOutput && typeof projectionOutput.then === 'function') {
          ...
          throw coordinatorError('AUTHORITY_TRANSACTION_ASYNC_CALLBACK_FORBIDDEN', 'Authority transaction projector returned a Promise');
        }
        if (!isPlainObject(projectionOutput)) {
          throw coordinatorError('AUTHORITY_TRANSACTION_PROJECTOR_RESULT_INVALID', 'Projector must return a plain result object');
        }
        const stateHash = requiredString(projectionOutput.stateHash, 'projector.stateHash', 64).toLowerCase();
        if (!/^[a-f0-9]{64}$/.test(stateHash)) {
          throw coordinatorError('AUTHORITY_TRANSACTION_PROJECTOR_HASH_INVALID', 'Projector stateHash must be SHA-256');
        }
