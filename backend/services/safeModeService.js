...
function authoritySnapshot() {
  ...
  return {
    ...
    operatingMode,
    reason: String(snapshot?.reasonCode || snapshot?.reason || ''),
    reasons: Array.isArray(snapshot?.reasons) ? [...snapshot.reasons] : [],
    enteredAt: String(snapshot?.enteredAt || ''),
    updatedAt: String(snapshot?.updatedAtUtc || ''),
    updatedBy: String(snapshot?.updatedBy || 'runtime-authority'),
    trigger: String(snapshot?.trigger || ''),
    evidenceSha256: String(snapshot?.evidenceSha256 || ''),
