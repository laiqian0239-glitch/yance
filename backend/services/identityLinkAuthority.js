...
class IdentityLinkAuthority {
  ...
  observeWithinTransaction(input = {}, repo = this.repository) {
    ...
    const personId = suppliedPersonId || stableId('person', [scope.workspaceId, scope.platform, scope.sourceAccountId, scope.externalId]);
    ...
    const identityLinkId = stableId('identity-link', [scope.workspaceId, scope.platform, scope.sourceAccountId, scope.externalId]);
  ...
  finalizeObservation(result = {}) {
    ...
    recordIdentityDomainEvent({
      repository: this.repository, eventType: pending.eventType, auditId: pending.auditId,
  ...
  transition(identityLinkId, nextStatus, input = {}) {
    ...
    return this.repository.transaction(repo => {
      ...
      recordIdentityDomainEvent({ repository: this.repository, eventType: 'identity.link.transitioned', auditId, workspaceId: link.workspace_id, platform: link.platform, sourceAccountId: link.source_account_id, projection: result, targetRefs: [{ table: 'identity_links', id: identityLinkId }, { table: 'identity_link_audit', id: auditId }] });
      return result;
    });
  }
  ...
  merge(input = {}) {
    ...
    return this.repository.transaction(repo => {
      ...
      recordIdentityDomainEvent({ repository: this.repository, eventType: 'identity.person.merged', auditId, workspaceId: source.workspace_id, platform: links[0]?.platform || 'identity', sourceAccountId: links[0]?.source_account_id || source.workspace_id, projection: result, targetRefs: [{ table: 'persons', id: sourcePersonId }, { table: 'persons', id: targetPersonId }, { table: 'identity_link_audit', id: auditId }] });
      return result;
    });
  }
  ...
  resolve(scope = {}) {
    const link = this.repository.getIdentityLinkByScope(validateScope(scope));
    return link ? { authority: AUTHORITY, person: publicPerson(this.repository.getPerson(link.person_id)), link: publicLink(link) } : null;
  }
}

const singleton = new IdentityLinkAuthority();
module.exports = { AUTHORITY, SCHEMA_VERSION, LINK_STATUS, IdentityLinkAuthority, singleton, validateScope, sanitizeIdentityObject };
