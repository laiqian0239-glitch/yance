...
class RelationshipKeyNodeRepository {
  ...
  /** Graphiti is the factual authority; this method only persists its compatibility projection. */
  projectGraphitiFacts({ contactId, conversationId, facts } = {}) {
    ...
    const statement = this.db.prepare(`
      ...
      ) VALUES (?, ?, ?, 'graphiti_fact', ?, ?, '{}', ?, ?, '[]', ?, 1.0, ?, 'graphiti:v0.29.3', ?, ?, 0, 'fact', '', '')
    ...
    for (const fact of rows) {
      ...
      const status = fact.invalidAt ? 'invalidated' : 'confirmed';
