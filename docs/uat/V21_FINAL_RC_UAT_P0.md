...
- Startup-capsule durable failure telemetry closure: PR #887 authorization + PR #888 implementation ordinary merge `7f73b95b12a906d87513e7ea222c3fb51fa54311`. The helper preserves structured `STARTUP_CAPSULE_*` failures at `startup-capsule/diagnostics/startup-capsule-failure.json` before nonzero exit whenever the output diagnostics seam exists.
...
- Frozen successor-v3 candidate evidence: PR #890 exact head `10d5eb2e20168fb0e1693befa161658dc188668f`; Product Final run `33052777045` had frozen Element and Matrix GREEN, and Windows job `98452076458` completed the single full packaged build plus real packaged launch before the same-build startup capsule failed only with `STARTUP_CAPSULE_SERVER_IMPORT_PROOF_MISSING`. This exact head and every artifact/receipt bound to it are stale and must not be rerun or mutated.
- Startup server-import proof closure: PR #891 authorization, test-only causal RED `ed1e756e05b341d25f96dfb1e25b8f6cbcf5ef60` / Stage `33056103527`, focused repair carried through frozen #892 and forward-continuation authorization #893, then ordinary-merged by PR #894 at `061d1101d1eb361d3f533077b9a01463b88c5d8a`. `verifyStartupDiagnostics` now consumes canonical `<dataRoot>/logs/production-diagnostics.jsonl`, allowing `backend-boot-started` to satisfy server-import proof without changing startup payloads or backend producers.
- Successor-v4 Product Final route authorization: PR #895 ordinary merge `b79e57cabdaa5529fcd5bc0f330923522bda23d6`.
- Successor-v4 Product Final route implementation: PR #896 exact final head `6fa28fb66f6a197a8a943fc1f2a5276e5d682326`, ordinary merge `8c8eec19fb6b3a09c898a8145a89cd3c49738259`; the only semantic change is exact equality admission of `release/v21-final-rc-uat-p0-successor-v4` to the three existing Product Final jobs, with job bodies unchanged.
- Clean successor-v4 authorization: PR #897 ordinary merge `b67bbfa39ba162a336c1023b487235cf3679ec85`, strict parents `8c8eec19fb6b3a09c898a8145a89cd3c49738259` + `6f8bb83e94db8d9d66fa4a688d19fbd7438c45da`.
- This marker commit is the direct child of authorization merge `b67bbfa39ba162a336c1023b487235cf3679ec85` and intentionally contains no `supersedes` edge to #874, #880, #881, #890, or their implementation branches.
- Exact candidate branch: `release/v21-final-rc-uat-p0-successor-v4`
...
The final candidate identity is the current immutable PR head after this marker-only successor commit. The document intentionally does not self-embed that head SHA because this marker commit itself advances the candidate. Final Delta, CI, Product Final artifacts, startup-capsule evidence, packaged launch receipt, review, and downstream UAT evidence must all bind to the repository-reported exact PR head. Any later candidate-byte or ancestry change invalidates prior final evidence and requires a fresh exact-head pass.
...
- the post-`375b99f7...` production continuation is explicitly classified, including the PersonalAccess startup repair, post-boot migration closure, durable startup-capsule telemetry, canonical `production-diagnostics.jsonl` server-import proof repair, and successor-v4 exact Product Final route
...
Only after fresh exact-head Product Final and startup-capsule validation pass may the complete 500+ MB RC be downloaded for UAT-001 / issue #1 on real Windows. That UAT still requires real-Windows real-provider model-worker acceptance on these exact new packaged bytes. The old `375b99f7...`, `04c0d5d5...`, `6ef44bd2...`, `d73c8809...`, and `10d5eb2e...` packages cannot satisfy that boundary. Real AppData/DB remains untouched until the exact new package is downloaded and the explicit UAT execution step is reached.
...
This clean successor-v4 is marker-only and begins from ordinary-merged authorization `b67bbfa39ba162a336c1023b487235cf3679ec85`. It changes no product/runtime/test/runner/workflow/routing/dependency/package/lockfile/database bytes. All Final Delta, CI, Product Final artifacts, startup-capsule evidence, packaged receipts, review, and downstream UAT evidence must bind to the resulting repository-reported exact head; every precursor exact-head result remains stale.
