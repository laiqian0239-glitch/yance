...
if (-not $isWindowsHost) {
    $version = & $Python -c $versionProbe 2>&1 | Out-String
...
$null = & $wsl.Source --status 2>&1 | Out-String
if ($LASTEXITCODE -ne 0) {
    throw 'TRAINING_RUNTIME_UNAVAILABLE: WSL status could not be verified.'
}

$null = & $wsl.Source --exec sh -lc 'case "$(uname -r)" in *WSL2*|*microsoft-standard-WSL2*) exit 0;; *) exit 1;; esac' 2>&1 | Out-String
if ($LASTEXITCODE -ne 0) {
...
$runtimeVersion = & $wsl.Source --exec env -i `
    ...
    $Python -c $versionProbe 2>&1 | Out-String
