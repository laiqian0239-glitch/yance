...
function verifyActive(root, expectedRefs, operationType) {
  const p = paths(root);
  const host = makeHost(root);
  const before = host.snapshotMetadata();
  const refs = host.refs().slice().sort();
  const frame = host.createHydrationFrame({
  ...
  const checks = {
    ...
    generationAdvancedOnceByFd5: before.generation === 0 && frame.frame.generation === 1 && after.generation === 1,
  ...
  return {
    ...
    frameEntryCount: frame.frame.payload.entries.length, activeTransactionId: after.activeTransactionId,
    backendFinalState: 'NOT_STARTED_AUTHORITY_ONLY', secretValueRecorded: false, secretHashRecorded: false
  };
}
function scenario(kind, point, entries) {
  ...
  try {
    ...
    const verified = verifyActive(root, expectedRefs, kind);
...
function baselineCase(id, kind, entries, createLegacyFile = false) {
...
function migrationFailurePreservesLegacyVaultCase() {
  ...
  try {
    ...
    try { recovery = verifyActive(root, entries.map(([ref]) => ref), 'MIGRATION'); }
...
function negativeCases() {
  ...
  for (const type of ['STRUCTURE_CORRUPTED', 'DECRYPT_FAILED']) {
    ...
    try {
      ...
      let reasonCode = '';
      try { makeHost(root); } catch (error) { reasonCode = error.reasonCode || error.code || ''; }
...
async function runCredentialAuthorityLifecycleMatrix() {
  const rows = [
    baselineCase('GENESIS_CLEAN_FIRST_START', 'GENESIS', []),
    baselineCase('MIGRATION_EMPTY_WP3_VAULT', 'MIGRATION', [], true),
    baselineCase('MIGRATION_MULTI_REFERENCE_WP3_VAULT', 'MIGRATION', [['legacy/one', { token: 'redacted-one' }], ['legacy/two', { token: 'redacted-two' }]])
  ];
  for (const point of GENESIS_POINTS) rows.push(scenario('GENESIS', point, []));
  for (const point of MIGRATION_POINTS) rows.push(scenario('MIGRATION', point, [['legacy/one', { token: 'redacted-one' }], ['legacy/two', { token: 'redacted-two' }]]));
  rows.push(migrationFailurePreservesLegacyVaultCase());
  rows.push(...negativeCases());
