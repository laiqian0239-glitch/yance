...
$electronArchiveExpectedSha256 = 'c2ef9a5f65472c34d14bd3e67b7d14e66b0c01f124aba45263d6a4232160e13a'
...
try {
  ...
  try {
    ...
    Write-Host 'Materialize isolated WP7 archive OSS toolchain...'
    $archiveOssRoot = Join-Path $EvidenceRoot 'wp7-archive-oss-toolchain'
    if (Test-Path -LiteralPath $archiveOssRoot) { throw "archive OSS toolchain directory must be new: $archiveOssRoot" }
    ...
    if (-not (Test-Path -LiteralPath $archiveManifestPath -PathType Leaf) -or -not (Test-Path -LiteralPath $archiveLockPath -PathType Leaf)) {
      throw 'tracked WP7 archive OSS manifest/lock is missing.'
    }
    ...
    if (-not (Test-Path -LiteralPath $archiveTarPackagePath -PathType Leaf)) { throw 'isolated tar package metadata is missing.' }
    ...
    if ([string]$archiveTarPackage.name -ne 'tar' -or [string]$archiveTarPackage.version -ne '7.5.22') {
      throw "archive OSS identity mismatch: $($archiveTarPackage.name)@$($archiveTarPackage.version)"
    }
