...
function compareElectronDistributionTree(options = {}) {
  ...
  const allowedAddition = (relative) => relative.startsWith('resources/app/') || relative.startsWith('resources/runtime/node22/') || relative.startsWith('resources/parlant-runtime/') || relative.startsWith('resources/learning-runtime/') || relative.startsWith('resources/matrix-runtime/') || metadata.has(relative);
  ...
  return Object.freeze({ archiveFileCount: official.length, modeBoundFileCount: official.filter((row) => Number(row.unixMode || 0) !== 0).length, distributionTreeSha256, records: official.map(({ payloadPath, ...row }) => ({ ...row, payloadPath })), allowedProductAdditions: ['resources/app/**', 'resources/runtime/node22/**', 'resources/parlant-runtime/**', 'resources/learning-runtime/**', 'resources/matrix-runtime/**', ...metadata] });
}
