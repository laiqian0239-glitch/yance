...
check('AR25', () => {
  ...
  const requiredDynamicCases = [
    ...
    'formal trusted-product probe IDs and the pre-entitlement/Product partition have one executable and governance authority',
    ...
    'complete pre-entitlement raw evidence validates all six results, logs, custody, context and offline proof while preserving the three entitled probes',
  ...
  if (packagedScript !== 'node tools/wp7/run-packaged-electron-probe-integration.js' || /\$WP7_|%WP7_|--installer-sha256|--probe-id\s+first-start/.test(packagedScript) || !runner.includes('--product-executable') || !runner.includes('--electron-archive') || !runner.includes('--pre-review-sealed-artifact') || !runner.includes('WP7_PACKAGED_PRODUCT_EXECUTABLE') || !runner.includes('WP7_ELECTRON_RELEASE_ARCHIVE') || !runner.includes('WP7_PRE_REVIEW_SEALED_ARTIFACT')) throw new Error('formal packaged product command or Windows-safe CLI/env authority is missing, narrowed or still accepts a bare installer hash');
  if ((!runner.includes('const probeIds = requestedProbeId ? [requestedProbeId] : PRE_ENTITLEMENT_PROBE_IDS') || !runner.includes('for (const probeId of probeIds)') || !runner.includes('ENTITLED_PRODUCT_PROBE_IDS.includes(requestedProbeId)') || !runner.includes('WP7_PRE_REVIEW_PRODUCT_ENTITLEMENT_REQUIRED')) || !runner.includes('producerPid: expected.producerPid') || !runner.includes('validateMeasurements(expected.probeId, report.measurements)') || !runner.includes('readAndVerifyPreReviewSealedArtifact') || !runner.includes('processCustodySha256') || !runner.includes('executionContextSha256') || !runner.includes("artifactClass !== PRE_REVIEW_ARTIFACT_CLASS") || runner.includes('electronReleaseArchivePath: trust.archivePath')) throw new Error('packaged product runner does not bind the six pre-entitlement results, preserve the three entitled Product probes, the actual sealed artifact file, relative raw evidence and Pre-Review-only classification');
  ...
  if (!evidencePackage.includes('validatePreEntitlementProbeRawEvidence') || !evidencePackage.includes('WP7_TRUSTED_PRODUCT_RAW_PROBE_EVIDENCE_MISSING') || !evidencePackage.includes('processCustodySha256') || !evidencePackage.includes('executionContextSha256') || !evidencePackage.includes('readPreMainProof') || !evidencePackage.includes('assertNoAbsoluteOrTempReferences')) throw new Error('raw pre-entitlement probe evidence, process custody, network isolation proof or relative-path verification is incomplete');
  ...
  if (!formalProbeAuthority.includes("'first-start'") || !formalProbeAuthority.includes("'boot-failure'")
      || formalProbeScope.schemaVersion !== 2
      || formalProbeScope.requiredProbeCount !== 9
      || formalProbeScope.preEntitlementProbeCount !== 6
      || formalProbeScope.entitledProductProbeCount !== 3
      || JSON.stringify(formalProbeScope.formalProbeIds) !== JSON.stringify(['first-start','controlled-stop','restart','offline-start','crash-recovery','safe-mode-negative','credential-gate-negative','event-gap-recovery','boot-failure'])
      || JSON.stringify(formalProbeScope.preEntitlementProbeIds) !== JSON.stringify(['first-start','offline-start','crash-recovery','safe-mode-negative','credential-gate-negative','boot-failure'])
      || JSON.stringify(formalProbeScope.entitledProductProbeIds) !== JSON.stringify(['controlled-stop','restart','event-gap-recovery'])
      || formalProbeScope.preReviewRunnerExecutesPreEntitlementOnly !== true
      || formalProbeScope.finalWindowsHarnessRequiresAllFormalProbes !== true
      || !blockerGenerator.includes('formalProbeIds: [...FORMAL_PROBE_IDS]')
      || !windowsHarness.includes('assertFormalProbeIdSet(configuredProbeIds)')) throw new Error('trusted-product executable probe IDs and 6/3 authority partition do not have one shared authority across governance, pre-review and final Windows execution');
  ...
  if (!preReviewEvidenceGenerator.includes('validatePreEntitlementProbeRawEvidence') || !preReviewEvidenceGenerator.includes('WP7_PRE_REVIEW_EVIDENCE_INDEX.json') || !preReviewEvidenceGenerator.includes('WP7_PRE_REVIEW_INTERNAL_SHA256.txt') || !preReviewEvidenceGenerator.includes('complete verification output')) throw new Error('Pre-Review evidence generator does not require actual pre-entitlement raw probes and complete verification outputs');
