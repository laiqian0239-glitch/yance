...
factory_path = Path('backend/services/whatsappSocketFactory.js')
factory_path.write_text(FACTORY, encoding='utf-8')

adapter_path = Path('backend/services/whatsappAdapter.js')
adapter = adapter_path.read_text(encoding='utf-8')

import_marker = "const { createWhatsAppMessageRetryStore } = require('./whatsappMessageRetryStore');\n"
import_replacement = import_marker + "const { createWhatsAppSocketFactory } = require('./whatsappSocketFactory');\n"
assert adapter.count(import_marker) == 1
adapter = adapter.replace(import_marker, import_replacement, 1)
...
old_block = """    const socketOptions = {
    ...
    const socket = baileys.default(socketOptions);
"""
new_block = """    const socketFactory = createWhatsAppSocketFactory({ baileys, logger });
...
"""
assert adapter.count(old_block) == 1
adapter = adapter.replace(old_block, new_block, 1)
assert "const socketOptions = {" not in adapter
assert "baileys.default(socketOptions)" not in adapter
assert "row.socket = socket" in adapter
