...
function isRetryableNpmAuditEndpointTransportFailure(result, parsed) {
  ...
  const hasHttpStatus = Object.prototype.hasOwnProperty.call(parsed, 'statusCode')
    && parsed.statusCode !== null
    && parsed.statusCode !== undefined;

  return parsed.method === 'POST'
    && parsed.uri === NPM_AUDIT_TRANSIENT_ENDPOINT
    && !hasHttpStatus;
}
    ...
    try {
      ...
      if (isRetryableNpmAuditEndpointTransportFailure(result, parsed)) {
        ...
        throw createNpmAuditEndpointResultError(
          ...
          'npm audit advisories-bulk endpoint transport failed after the bounded whole-audit retry',
          result,
          parsed,
          attempt
        );
