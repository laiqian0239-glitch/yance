...
const NPM_AUDIT_TRANSIENT_ENDPOINT = 'https://registry.npmjs.org/-/npm/v1/security/advisories/bulk';
const NPM_AUDIT_TRANSIENT_NETWORK_TIMEOUT_MESSAGE = `network timeout at: ${NPM_AUDIT_TRANSIENT_ENDPOINT}`;
...
function isRetryableNpmAuditEndpointNetworkTimeout(result) {
  if (!Number.isInteger(result?.status) || result.status === 0) return false;
  const parsed = parseNpmAuditResultObject(result);
  return Boolean(
    parsed
    && parsed.message === NPM_AUDIT_TRANSIENT_NETWORK_TIMEOUT_MESSAGE
    && parsed.error
...
function createNpmAuditEndpointNetworkTimeoutError(result) {
  return createGovernanceError(
    'WP_B_UPSTREAM_NPM_AUDIT_ENDPOINT_NETWORK_TIMEOUT',
    'npm audit advisories-bulk endpoint timed out after the bounded whole-audit retry',
    {
      ...
      attempts: NPM_AUDIT_MAX_ATTEMPTS
    }
  );
}
...
function runGovernedNpmAudit(auditRoot, options = {}) {
  ...
  for (let attempt = 1; attempt <= NPM_AUDIT_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!isRetryableNpmAuditEndpointNetworkTimeout(result)) return result;

      if (attempt === NPM_AUDIT_MAX_ATTEMPTS) {
        throw createNpmAuditEndpointNetworkTimeoutError(result);
      }
    } catch (error) {
      if (
