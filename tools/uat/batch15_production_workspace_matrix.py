...
() => {
  ...
  const systemOverview = {
    at:new Date().toISOString(),product:{name:'言策',version:'1.0.0',build:'Batch15 Matrix'},
...
def run(output: Path, screenshots: Path | None = None) -> dict[str, Any]:
        ...
        for width, height in VIEWPORTS:
            ...
            for zoom in ZOOMS:
                ...
                for route, workspace_id, body_selector in ROUTES:
                    integrity = page.evaluate("route => window.YanceWorkspaceRouteAuthority.applyRoute(document.getElementById('app'), route, {source:'batch15-matrix'})", route)
                    ...
                    if screenshots and width == 1512 and height == 931 and zoom == 1.0 and route in {"accounts", "system", "insights", "ai-workbench", "settings"}:
                        page.locator(f"#{workspace_id}").screenshot(path=str(screenshots / f"batch15-{route}.png"))
    ...
    report = {
        "schemaVersion": 1,
        "documentType": "YANCE_BATCH15_PRODUCTION_WORKSPACE_MATRIX",
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(REPO / "evidence" / "YANCE_BATCH15_PRODUCTION_WORKSPACE_MATRIX.json"))
