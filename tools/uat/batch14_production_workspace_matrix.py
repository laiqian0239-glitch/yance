#!/usr/bin/env python3
"""Render the production DOM with the real page templates and final CSS authority.
...
() => {
  ...
  const systemOverview = {
    at:new Date().toISOString(),product:{name:'言策',version:'1.0.0',build:'Batch14 Matrix'},
...
({workspaceId, bodySelector}) => {
  ...
  return {
    ...
    body:{width:bodyRect.width,height:bodyRect.height}, visibleScrollableCount:visibleScrollable.length, nestedVerticalScrollChains,directOverlap,obscured,routeSpecific
  };
}
"""
...
def run(output: Path, screenshots: Path | None = None) -> dict[str, Any]:
        ...
        for width, height in VIEWPORTS:
            ...
            for zoom in ZOOMS:
                ...
                for route, workspace_id, body_selector in ROUTES:
                    integrity = page.evaluate("route => window.YanceWorkspaceRouteAuthority.applyRoute(document.getElementById('app'), route, {source:'batch14-matrix'})", route)
                    ...
                    if screenshots and width == 1512 and height == 931 and zoom == 1.0 and route in {"accounts", "system", "insights", "ai-workbench", "settings"}:
                        page.locator(f"#{workspace_id}").screenshot(path=str(screenshots / f"batch14-{route}.png"))
    ...
    report = {
        "schemaVersion": 1,
        "documentType": "YANCE_BATCH14_PRODUCTION_WORKSPACE_MATRIX",
        ...
        "limitations": ["Chromium DOM preflight; real Windows Electron and real platform sessions remain required"],
        "results": results,
    }
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(REPO / "evidence" / "YANCE_BATCH14_PRODUCTION_WORKSPACE_MATRIX.json"))
