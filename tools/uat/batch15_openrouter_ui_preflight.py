#!/usr/bin/env python3
"""Chromium interaction preflight for the Batch15 OpenRouter cloud-first flow.
...
OVERRIDE = r"""
() => {
  window.__batch15Events = [];
  window.__batch15Status = [];
  ...
  window.YanceSystemStatus = { show: (type, message) => window.__batch15Status.push({type, message:String(message||'')}) };
  ...
  window.yanceDesktop.saveCredential = async (ref, value) => {
    window.__batch15Events.push({type:'credential-save',ref:String(ref||''),keyLength:String(value?.apiKey||'').length,hasBearerPrefix:/^Bearer\s/i.test(String(value?.apiKey||''))});
  ...
  const statusPayload = () => ({
    ok:true,models:[cloudModel,localModel],routes,summary:{count:2,online:2,verified:2,routingEligible:2,openRouterConnected:true},
  ...
  window.fetch = async (url, options={}) => {
    ...
    window.__batch15Events.push(record);
    if(value.includes('/api/r32/models/cloud/openrouter/auto-configure')) return jsonResponse({ok:true,snapshot:{modelCount:80,eligibleModelCount:45,freeModelCount:12,shortlistedModelCount:8,registeredModelCount:8},models:[cloudModel,localModel],routes,replyBrain:statusPayload().replyBrain});
...
def run(output: Path, screenshot_dir: Path | None = None) -> dict[str, Any]:
    ...
    with sync_playwright() as p:
        ...
        for width, height in VIEWPORTS:
            ...
            if width == 1512 and height == 931:
                if screenshot_dir:
                    dialog.screenshot(path=str(screenshot_dir / 'batch15-openrouter-dialog.png'))
                ...
                events = page.evaluate("() => window.__batch15Events")
                status_events = page.evaluate("() => window.__batch15Status")
                ...
                if flow and (not flow['dialogClosed'] or not flow['keyInputCleared']):
                    flow['pass'] = False
            results.append({'viewport': f'{width}x{height}', 'firstAction': first_action, 'layout': metrics, 'layoutPass': layout_pass, 'flow': flow, 'pass': bool(layout_pass and first_action == '一键接入 OpenRouter' and (flow is None or flow['pass']))})
    ...
    report = {
        'schemaVersion': 1,
        'documentType': 'YANCE_BATCH15_OPENROUTER_UI_PREFLIGHT',
        ...
        'limitations': ['Mocked OpenRouter responses; real Windows Electron, real credential vault, real network and real OpenRouter account remain required'],
        'results': results
    }
...
def main() -> int:
    ...
    parser.add_argument('--output', default=str(Path.cwd() / 'evidence' / 'YANCE_BATCH15_OPENROUTER_UI_PREFLIGHT.json'))
