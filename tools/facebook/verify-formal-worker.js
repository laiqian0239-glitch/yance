...
const EXPECTED_OAUTH_CONTRACT_VERSION = 5;
...
async function verify(fetchImpl = fetch) {
  ...
  try {
    ...
    if (!validContract) {
      throw Object.assign(new Error(`正式 Facebook Worker OAuth 合同过旧或不完整（HTTP ${response.status}）`), {
        code: 'FACEBOOK_FORMAL_WORKER_OAUTH_CONTRACT_STALE', status: response.status, response: data
      });
    }
    return {
...
module.exports = { WORKER_BASE_URL, EXPECTED_SERVICE, EXPECTED_GRAPH_VERSION, EXPECTED_CALLBACK_URL, EXPECTED_OAUTH_CONTRACT_VERSION, verify };
