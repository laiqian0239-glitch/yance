...
function makeCleanClone() {
  ...
  if (!sourceBranch) execFileSync('git', ['checkout', '--quiet', '--detach', sourceCommit], { cwd: repo });
...
test('historical evidence succeeds only when tests and generator run inside a detached worktree at that commit', () => {
  ...
  execFileSync('git', ['config', 'user.name', 'WP0 Test'], { cwd: repo });
  execFileSync('git', ['config', 'user.email', 'wp0-test@example.invalid'], { cwd: repo });
  execFileSync('git', ['commit', '--allow-empty', '--quiet', '-m', 'temporary newer commit'], { cwd: repo });
  assert.notEqual(git(repo, ['rev-parse', 'HEAD']), historical);
  const worktree = path.join(root, 'historical-worktree');
  execFileSync('git', ['-c', 'core.autocrlf=false', '-c', 'core.eol=lf', 'worktree', 'add', '--quiet', '--detach', worktree, historical], { cwd: repo, encoding: 'utf8' });
