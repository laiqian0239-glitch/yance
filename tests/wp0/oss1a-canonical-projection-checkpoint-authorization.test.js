...
test('historical v8 registry snapshot remains valid after v9 selection changes', () => {
  assert.equal(validateOpenSourceWorkPackageRegistry(V8_REGISTRY), true);
});

test('historical v8 adds only the canonical projection checkpoint hash root path', () => {
  ...
  assert.deepEqual(authorization.exactPaths.filter(value => !previous.exactPaths.includes(value)), [ROOT_REPAIR_PATH]);
