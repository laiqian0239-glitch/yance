...
for (const locale of ['de-DE', 'de-AT']) {
  test(`V21 Persona P0: ${locale} native WhatsApp register rejects translationese and assistant-style prose`, () => {
    ...
    assert.equal(contract.locale, locale);
    assert.equal(contract.channel, 'whatsapp');
    assert.equal(contract.register, 'native_short_form');
    assert.equal(contract.maxQuestions, 1);
    assert.ok(contract.prefer.some(rule => /kurz|natürlich|chat/i.test(rule)));

    const forbidden = contract.reject.join(' | ').toLowerCase();
    for (const marker of ['translationese', 'customer-service', 'email', 'over-explaining', 'repeated questions', 'emoji overuse', 'neediness', 'assistant clichés']) {
      assert.ok(forbidden.includes(marker), `native register must reject ${marker}`);
    }
  });
}
