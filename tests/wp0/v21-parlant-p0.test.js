...
test('Daily Chat Goal validates real calendar dates at Electron runtime and Parlant server seams', () => {
  const runtime = readText('electron/parlantRelationshipRuntime.js');
  const server = readText('runtime/parlant/yance_parlant_server.py');

  assert.match(runtime, /Date\.UTC\(year,\s*month - 1,\s*day\)/u);
  assert.match(runtime, /getUTCFullYear\(\)\s*!==\s*year/u);
  assert.match(runtime, /getUTCMonth\(\)\s*!==\s*month - 1/u);
  assert.match(runtime, /getUTCDate\(\)\s*!==\s*day/u);

  assert.match(server, /DAILY_LOCAL_DATE_PATTERN\.fullmatch\(value\)/u);
  assert.match(server, /date\.fromisoformat\(value\)/u);
  assert.match(server, /YANCE_PARLANT_DAILY_LOCAL_DATE_INVALID/u);
});
