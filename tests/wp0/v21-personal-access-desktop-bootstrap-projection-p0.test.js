...
test('main activation and post-install PASS remain independent from human entitlement after trusted bootstrap', () => {
  ...
  const receiptSource = read('electron/postInstallLaunchReceipt.js');
  ...
  assert.match(mainSource, /writePostInstallLaunchReceipt\(/u);
  assert.match(mainSource, /post-install-launch\.json/u);
  assert.match(receiptSource, /documentType:\s*'YANCE_POST_INSTALL_LAUNCH_RECEIPT'/u);
  assert.match(receiptSource, /\?\s*'PASS'\s*:\s*'FAIL'/u,
...
test('existing WP6 real-client harness accepts both bootstrap and product snapshot URLs without scope expansion', () => {
  const helperSource = read('tests/wp6/helpers.js');
  assert.match(helperSource, /url\.includes\('\/snapshot'\)/u,
    'existing WP6 client harness must remain compatible with the new bootstrap snapshot path without modifying WP6 tests');
});
