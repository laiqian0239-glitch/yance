...
test('delegated authorization with mismatched supersedes implementationBranch is graph-invalid', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'yance-delegated-graph-invalid-'));
  const git = (...args) => execFileSync('git', args, {
    cwd: root,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  ...
  try {
    ...
    const v1AuthorizationPath = 'governance/layered-ci/graph-invalid-v1-authorization.json';
    const v1AuthorizationBranch = 'governance/graph-invalid-v1-authorization';
    const v1ImplementationBranch = 'fix/graph-invalid-v1';
    const v1Authorization = genericDelegatedAuthorization({
      allowedChangedPaths,
      document: {
        workPackage: 'GRAPH-INVALID-V1',
        base: { branch: 'main', commit: base },
        authorizationBranch: {
          name: v1AuthorizationBranch,
          allowedChangedPaths: [v1AuthorizationPath],
    ...
    const v2AuthorizationPath = 'governance/layered-ci/graph-invalid-v2-authorization.json';
    const v2AuthorizationBranch = 'governance/graph-invalid-v2-authorization';
    const v2ImplementationBranch = 'fix/graph-invalid-v2';
    const v2Authorization = genericDelegatedAuthorization({
      allowedChangedPaths,
      document: {
        workPackage: 'GRAPH-INVALID-V2',
        supersedes: {
          authorizationPath: v1AuthorizationPath,
          implementationBranch: 'fix/MISMATCHED-BRANCH-NAME',
          reason: 'intentionally mismatched implementationBranch causes graph invalid'
        },
        base: { branch: 'main', commit: v1Merge },
        authorizationBranch: {
          name: v2AuthorizationBranch,
    ...
    write(v2AuthorizationPath, `${JSON.stringify(v2Authorization, null, 2)}\n`);
    git('add', v2AuthorizationPath);
    git('commit', '-m', 'authorize v2 with mismatched supersedes');
    const v2ReviewedHead = git('rev-parse', 'HEAD');
    ...
    const graphInvalid = evaluateTrustedDelegatedGovernanceBranch({
      branch: v2ImplementationBranch,
      trustedPolicyRoot: root,
      trustedMainHead,
      evaluatedHead: trustedMainHead
    });
    assert.equal(graphInvalid.pass, false, JSON.stringify(graphInvalid));
    assert.equal(graphInvalid.reasonCode, 'WP0_DELEGATED_GOVERNANCE_SUPERSESSION_INVALID');
    assert.equal(graphInvalid.authorizationPath, v2AuthorizationPath);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
