...
test('Windows UAT runner is verify-only and starts an image-only Matrix compose with no user-machine build or package resolution', () => {
  ...
  assert.doesNotMatch(runner, /(?m)^\s*(?:&\s*)?(?:npm(?:\.cmd)?|pnpm|node-gyp)(?:\.cmd|\.exe)?\s+/iu);
  assert.doesNotMatch(runner, /(?m)^\s*(?:&\s*)?docker(?:\.exe)?\s+build\b/iu);
  assert.doesNotMatch(runner, /(?m)^\s*(?:&\s*)?docker(?:\.exe)?\s+compose[^\n]*\sbuild(?:\s|$)/iu);
