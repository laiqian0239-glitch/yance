...
test('OSS-1A successor registry selects exact v5 authority and receipt paths', () => {
...
test('v5 receipt seals the immutable authorization anchor before implementation continuation', () => {
  assert.equal(fs.existsSync(path.join(REPO_ROOT, ...RECEIPT_PATH.split('/'))), true, 'v5 receipt must exist');
  const registry = loadOpenSourceWorkPackageRegistry();
  const entry = selectOpenSourceWorkPackageRegistryEntry(registry, IMPLEMENTATION_BRANCH);
  const authorization = readJson(AUTHORIZATION_PATH);
...
test('v5 governance is negative-proof only while implementation branch remains executable', () => {
