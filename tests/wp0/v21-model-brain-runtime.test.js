...
test('sealed Windows runtime preserves isolated import stderr in failure evidence',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1');assert.match(b,/\$importOutput\s*=.*2>&1/iu);assert.match(b,/isolated Router\/ComplexityRouter\/tag_filtering import gate failed:\s*\$\(\$importOutput/iu);});

test('sealed Windows runtime preserves the exact pinned LiteLLM source tree without a Yance fork',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml'),plan=read('docs/superpowers/plans/2026-08-09-yance-v21-model-brain-p0.md');assert.match(b,/source tree integrity|sourceTreePreserved/iu);assert.doesNotMatch(b,/SharedProxyFiles|SdkInitPatch|proxyCliImport|retainedSharedProxyModules|sdkInitPatch|sdkInitSourceBlob/u);assert.doesNotMatch(w,/allowedProxyFiles|shared proxy import allowlist/iu);assert.doesNotMatch(plan,/litellm\/proxy[^\n]*(?:rejected|removed)/iu);});

test('sealed Windows runtime materializes exact Git blob bytes without archive or worktree conversion',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1');assert.match(b,/cat-file\s+blob/iu);assert.match(b,/RedirectStandardOutput|StandardOutput\.BaseStream/iu);assert.doesNotMatch(b,/git\s+-C\s+\$source\s+archive/iu);assert.doesNotMatch(b,/Copy-Item\s+\(Join-Path\s+\$source\s+["']litellm["']\)\s+\$litellmTarget\s+-Recurse/iu);});

test('sealed Windows runtime forbids Proxy extras and service activation without deleting upstream SDK modules',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml'),worker=read('runtime/model-brain/yance_litellm_worker.py');assert.match(b,/litellm\[proxy\]|litellm-enterprise|litellm-proxy-extras/iu);assert.match(w,/service entrypoint|service activation|console entrypoint/iu);assert.match(w,/litellm-enterprise|litellm-proxy-extras/iu);assert.doesNotMatch(w,/Remove-Item\s+\$proxyRoot|allowedProxyFiles/iu);assert.doesNotMatch(worker,/run_server|proxy_server|proxy_cli/iu);});

test('sealed source integrity is verified against pinned Git blobs rather than worktree diff semantics',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1');assert.match(b,/ls-tree\s+-r[^\n]*\$ExpectedCoreTree/iu);assert.match(b,/hash-object\s+--no-filters/iu);assert.match(b,/expected.*blob|blob.*mismatch/iu);assert.match(b,/unexpected.*source|source.*path.*mismatch/iu);assert.doesNotMatch(b,/diff\s+--no-index/iu);});

test('offline Windows gate uses a runner-compatible firewall rule lifecycle without unsupported passthrough semantics',()=>{const w=read('.github/workflows/v21-model-brain-p0-windows.yml');assert.match(w,/\$firewallRuleName\s*=\s*["']YanceModelBrainOffline["']/iu);assert.match(w,/New-NetFirewallRule[^\n]*-DisplayName\s+\$firewallRuleName[^\n]*-Program\s+\$python/iu);assert.doesNotMatch(w,/New-NetFirewallRule[^\n]*-PassThru/iu);assert.match(w,/Remove-NetFirewallRule\s+-DisplayName\s+\$firewallRuleName/iu);});

test('provider failures and child stderr cannot project raw untrusted text into public Model Brain status',()=>{const r=read('backend/services/modelBrainRuntime.js'),w=read('runtime/model-brain/yance_litellm_worker.py');assert.match(w,/isinstance\(exc,\s*ModelBrainError\)/u);assert.match(w,/Model Brain provider request failed/u);assert.doesNotMatch(w,/["']message["']:\s*_clean\(exc\)\[:800\]/u);assert.match(r,/MODEL_BRAIN_RUNTIME_STDERR/u);assert.doesNotMatch(r,/fail\(["']MODEL_BRAIN_RUNTIME_STDERR["']\s*,\s*message\)/u);});
