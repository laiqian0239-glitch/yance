...
test('sealed Windows runtime preserves the exact pinned LiteLLM source tree without a Yance fork',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml'),plan=read('docs/superpowers/plans/2026-08-09-yance-v21-model-brain-p0.md');assert.match(b,/Copy-Item\s+\(Join-Path\s+\$source\s+["']litellm["']\)\s+\$litellmTarget\s+-Recurse/iu);assert.match(b,/source tree integrity|sourceTreePreserved/iu);assert.doesNotMatch(b,/SharedProxyFiles|SdkInitPatch|proxyCliImport|retainedSharedProxyModules|sdkInitPatch|sdkInitSourceBlob/u);assert.doesNotMatch(w,/allowedProxyFiles|shared proxy import allowlist/iu);assert.doesNotMatch(plan,/litellm\/proxy[^\n]*(?:rejected|removed)/iu);});
