...
test('sealed Windows runtime keeps only LiteLLM upstream shared proxy import closure',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml');assert.match(b,/SharedProxyFiles/u);for(const x of ['proxy/__init__.py','proxy/_types.py','proxy/types_utils/utils.py','proxy/spend_tracking/cold_storage_handler.py']){const rx=new RegExp(x.replaceAll('/','[/\\\\]'),'u');assert.match(b,rx);assert.match(w,rx);}assert.match(w,/allowedProxyFiles/u);assert.doesNotMatch(w,/FullName\s+-match\s+["'][^"']*litellm\[\\\\\/\]proxy\[\\\\\/\]/iu);assert.match(w,/proxy_server|litellm-proxy-extras/iu);});

test('sealed base SDK deterministically removes only the upstream Proxy CLI top-level import',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml');assert.match(b,/3f8c742c5a2462ec0c08ed24e774bb88e303752f/u);assert.match(b,/SdkInitPatch/u);assert.match(b,/from \.proxy\.proxy_cli import run_server/u);assert.match(b,/sdkInitPatch/u);assert.match(b,/sdkInitSourceBlob/u);assert.doesNotMatch(b,/["']proxy\/proxy_cli\.py["']/u);assert.doesNotMatch(w,/["']proxy\/proxy_cli\.py["']/u);});
