...
test('sealed Windows runtime keeps only LiteLLM upstream shared proxy type closure',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1'),w=read('.github/workflows/v21-model-brain-p0-windows.yml');assert.match(b,/SharedProxyFiles/u);for(const x of ['proxy/__init__.py','proxy/_types.py','proxy/types_utils/utils.py'])assert.match(b,new RegExp(x.replaceAll('/','[/\\\\]'),'u'));assert.match(w,/allowedProxyFiles/u);assert.doesNotMatch(w,/FullName\s+-match\s+["'][^"']*litellm\[\\\\\/\]proxy\[\\\\\/\]/iu);assert.match(w,/proxy_server|litellm-proxy-extras/iu);});
