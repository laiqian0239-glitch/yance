...
test('sealed Windows runtime materializes the pinned LiteLLM tree from Git objects, not checkout bytes',()=>{const b=read('tools/model-brain/build-windows-runtime.ps1');assert.match(b,/git\s+-C\s+\$source\s+archive[^\n]*\$ExpectedCoreTree/iu);assert.match(b,/tar\s+-xf\s+\$sourceArchive[^\n]*-C\s+\$litellmTarget/iu);assert.doesNotMatch(b,/Copy-Item\s+\(Join-Path\s+\$source\s+["']litellm["']\)\s+\$litellmTarget\s+-Recurse/iu);});
