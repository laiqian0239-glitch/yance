...
test('failure-first V3 opt-in marker must be exactly true when present', () => {
  const document = authorization({ failureFirstCommit: { freshCausalRedRequired: false } });
...
test('legacy failureFirstCommit declarations without the V3 opt-in marker preserve existing semantics', () => {
  const document = authorization();
  delete document.implementation.failureFirstCommit.freshCausalRedRequired;
