...
test('release workflow downloads and verifies the same-source sealed Matrix runtime before Final Builder', () => {
  ...
  assert.match(workflow, /gh api "repos\/\$env:GITHUB_REPOSITORY\/actions\/runs\/\$env:MATRIX_RUN_ID"/u);
  assert.match(workflow, /gh run download \$env:MATRIX_RUN_ID[^\n]*--name \$artifactName[^\n]*--dir \$root/u);
  assert.match(workflow, /Product-Experience-Materialized-Matrix-UAT-\$\(\$run\.head_sha\)/u);
  ...
  assert.match(workflow, /\$builderArgs\.RequireSignedInstaller = \$true/u);
