...
test('vault epoch change is denied unless legal reset authorization matches old and new epoch', async () => {
  ...
  const initial = await vaultHost.createHydrationFrame({ startupNonce: 'initial', oneTimeToken: 'z'.repeat(43), backendPid: process.pid, manifestSha256: 'a'.repeat(64) });
  ...
  const prepared = await vaultHost.createHydrationFrame({ startupNonce: 'nonce', oneTimeToken: 'x'.repeat(43), backendPid: process.pid, manifestSha256: 'a'.repeat(64) });
