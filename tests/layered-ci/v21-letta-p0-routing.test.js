...
const V21_LETTA_P0_BOOTSTRAP_PATHS = Object.freeze([
...
test('V2.1 Letta P0 bootstrap paths are exact PRODUCT_WP0 routes', () => {
  assert.equal(new Set(V21_LETTA_P0_BOOTSTRAP_PATHS).size, 3);
  for (const file of V21_LETTA_P0_BOOTSTRAP_PATHS) {
  ...
  const aggregate = classifyWp0Route(policy, V21_LETTA_P0_BOOTSTRAP_PATHS);
...
test('V2.1 Letta route bootstrap does not authorize broad product prefixes', () => {
  for (const prefix of ['config/', 'third_party/']) {
    assert.equal(policy.productPrefixes.includes(prefix), false, prefix);
  }
});

test('adjacent unregistered Letta-like paths remain fail closed', () => {
  for (const file of [
    'config/upstreams/v21-letta-unapproved.json',
    'third_party/licenses/letta-agent-sdk-unapproved.txt',
    'third_party/licenses/letta-code-unapproved.txt'
  ]) {
    const result = classifyWp0Route(policy, [file]);
  ...
  const malformed = classifyWp0Route(policy, ['../config/upstreams/v21-letta-p0.json']);
  assert.equal(malformed.pass, false, JSON.stringify(malformed));
  assert.equal(malformed.reasonCode, 'WP0_ROUTE_PATH_INVALID');
});

test('V2.1 Letta route bootstrap preserves governance and product-documentation precedence', () => {
  ...
  const documentation = classifyWp0Route(policy, ['docs/superpowers/plans/2026-08-07-letta-route-bootstrap.md']);
