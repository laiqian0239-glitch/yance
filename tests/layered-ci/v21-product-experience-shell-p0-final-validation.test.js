...
test('Product Experience final validation is an exact-head same-repository pull-request workflow with Windows and Linux trusted-CI materializers', () => {
  ...
  assert.match(source, /github\.event\.pull_request\.head\.ref\s*==\s*'fix\/v21-invite-only-product-login-ci-red-closure-p0-successor-v2'/u);
  ...
  assert.deepEqual([...allowedBranches].sort(), [
    ...
    'fix/v21-invite-only-product-login-ci-red-closure-p0-successor-v2',
  ...
  assert.equal(
    (source.match(/github\.event\.pull_request\.head\.ref\s*==\s*'fix\/v21-product-final-materialization-reconciliation-p0-successor-v2'/gu) || []).length,
    3,
    'Product reconciliation successor-v2 must be admitted by exactly the three existing Product Final job guards'
  );
  assert.equal(
  ...
  assert.equal(
    (source.match(/github\.event\.pull_request\.head\.ref\s*==\s*'fix\/v21-invite-only-product-login-ci-red-closure-p0-successor-v2'/gu) || []).length,
    3,
    'invite-only Product login CI RED closure successor-v2 must be admitted by exactly the three existing Product Final job guards'
  );
...
test('auth-entry successor-v2 is admitted by exactly the three Product Final job guards', () => {
  ...
  assert.equal(
    ...
    3,
    'auth-entry successor-v2 must be admitted by exactly the three Product Final jobs'
  );
  assert.doesNotMatch(
    source,
...
test('Product reconciliation successor-v2 is admitted by exactly the three Product Final job guards', () => {
  const source = readWorkflow();
  const exact = "github.event.pull_request.head.ref == 'fix/v21-product-final-materialization-reconciliation-p0-successor-v2'";
  assert.equal(
    source.split(exact).length - 1,
    3,
    'Product reconciliation successor-v2 must be admitted by exactly the three Product Final jobs'
  );
  assert.doesNotMatch(
    source,
    /head\.ref[\s\S]{0,80}(?:startsWith|contains|matches).*product-final-materialization-reconciliation/iu
  );
});
