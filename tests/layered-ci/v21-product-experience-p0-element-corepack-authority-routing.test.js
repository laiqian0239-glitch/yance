...
const COREPACK_AUTHORITY_PATCH = 'upstream-patches/element-web/0002-yance-package-manager-authority.patch';

test('V2.1 Element Corepack authority patch is an exact PRODUCT_WP0 route', () => {
  const result = classifyWp0Route(policy, [COREPACK_AUTHORITY_PATCH]);
  assert.equal(result.pass, true, JSON.stringify(result));
  assert.equal(result.route, ROUTES.PRODUCT);
  assert.equal(result.productChangesPresent, true);
  assert.equal(policy.productExactPaths.includes(COREPACK_AUTHORITY_PATCH), true);
});

test('Element Corepack route bootstrap does not authorize broad upstream patch prefixes', () => {
...
test('adjacent unregistered Element Corepack patch paths remain fail closed', () => {
  for (const file of [
    'upstream-patches/element-web/0003-yance-package-manager-authority.patch',
    'upstream-patches/element-web/0002-yance-package-manager-authority-extra.patch',
    'upstream-patches/other/0002-yance-package-manager-authority.patch'
  ]) {
  ...
  const malformed = classifyWp0Route(policy, ['../upstream-patches/element-web/0002-yance-package-manager-authority.patch']);
  assert.equal(malformed.pass, false, JSON.stringify(malformed));
  assert.equal(malformed.reasonCode, 'WP0_ROUTE_PATH_INVALID');
});

test('Element Corepack route bootstrap preserves governance and product-documentation precedence', () => {
