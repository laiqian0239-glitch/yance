...
const authorization = JSON.parse(fs.readFileSync(
  path.join(ROOT, 'governance/layered-ci/v21-product-ui-authority-consistency-wp3-routing-prerequisite-authorization.json'),
  'utf8'
));

const WP3_BOOTSTRAP_PATHS = Object.freeze([
  'integration/element-module/src/product-experience/PersonalAccessSurface.tsx'
]);
const WP3_BOOTSTRAP_PATH_SET_SHA256 = '7c434369cc8a60b374da1dbd85ffaab4cb40e6312fe2ce8dac730c0237c90e10';
const RED_TEST_PATHS = Object.freeze([
  'tests/layered-ci/governance-policy.test.js',
  'tests/layered-ci/v21-product-ui-authority-consistency-wp3-routing.test.js'
]);
const RED_TEST_PATH_SET_SHA256 = 'c65a23f0540c7bbba48ee7b4592282c11318292ca2e1a7a6fd257ea4bec280f2';
...
test('WP3 Personal Access surface is causal RED until exact PRODUCT_WP0 registration lands', () => {
  assert.equal(new Set(WP3_BOOTSTRAP_PATHS).size, 1);
  const file = WP3_BOOTSTRAP_PATHS[0];
...
test('WP3 routing authorization freezes the exact Product bootstrap and tests-only first head', () => {
  assert.deepEqual(authorization.bootstrapPaths, WP3_BOOTSTRAP_PATHS);
  assert.equal(authorization.bootstrapPathCount, 1);
  assert.equal(authorization.bootstrapPathSetSha256, WP3_BOOTSTRAP_PATH_SET_SHA256);
  assert.equal(canonicalPathSetSha256(WP3_BOOTSTRAP_PATHS), WP3_BOOTSTRAP_PATH_SET_SHA256);

  assert.equal(authorization.implementation.branch, 'fix/v21-product-ui-authority-consistency-wp3-routing-prerequisite');
...
test('WP3 Product bootstrap forbids broad prefixes and adjacent unregistered lookalikes', () => {
  ...
  for (const file of [
    'integration/element-module/src/product-experience/PersonalAccessSurface.unapproved.tsx',
  ...
  const malformed = classifyWp0Route(policy, ['../integration/element-module/src/product-experience/PersonalAccessSurface.tsx']);
  assert.equal(malformed.pass, false, JSON.stringify(malformed));
  assert.equal(malformed.reasonCode, 'WP0_ROUTE_PATH_INVALID');
});

test('WP3 route bootstrap preserves governance and product-documentation precedence', () => {
  ...
  const documentation = classifyWp0Route(policy, ['docs/superpowers/plans/2026-08-22-wp3-routing-prerequisite.md']);
...
test('WP3 route bootstrap reuses the trusted generic exact mutation guard', () => {
