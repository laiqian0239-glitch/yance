...
function accessMessage(status: PersonalAccessStatus | null): string {
  ...
  if (status.role === "OWNER") return "当前账号拥有永久使用权限；言策已开放。";
  if (status.usable) return "当前设备授权有效；言策已开放。";
  switch (status.reasonCode) {
    ...
    case "MATRIX_OPENID_TOKEN_REQUIRED": return "账号身份确认暂不可用；言策暂不进入主界面。";
    case "MATRIX_OPENID_SERVER_MISMATCH":
    case "MATRIX_SUBJECT_MISMATCH": return "账号身份与设备授权不匹配；言策暂不进入主界面。";
    case "UNKEY_AUTHORITY_UNAVAILABLE":
    case "UNKEY_AUTHORITY_REJECTED": return "权限服务暂不可用；为保护设备授权，言策暂不进入主界面。";
    ...
    case "UNKEY_KEY_ID_MISMATCH": return "当前设备授权不可用；言策暂不进入主界面。";
    case "DESKTOP_BRIDGE_UNAVAILABLE": return "个人使用权限服务暂不可用；言策暂不进入主界面。";
    case "ELEMENT_MATRIX_OPENID_SEAM_MISSING": return "账号身份确认暂不可用；言策暂不进入主界面。";
  ...
  const accessPanel = (
    <section className="yance-personal-access" aria-label="个人使用权限">
      <header className="yance-personal-access__header">
        <div><strong>个人使用权限</strong><span>{isOwner ? "永久权限" : "账号身份 + 设备授权"}</span></div>
