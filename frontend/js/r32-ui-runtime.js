...
function installR32Handlers(){
  ...
  function classifyRuntimeFailure(value,options={}){return window.YanceRuntimeErrors?.classifyRuntimeFailure?.(value,options)||{reasonCode:runtimeErrorCode(value)||'RENDERER_RUNTIME_FAILURE',severity:'error',fatal:true,recoverable:false,silent:false,userMessage:userFacingRuntimeError(value,'运行时异常')}}
  function recordRendererRuntimeFailure(kind,value,event={}){const evidence=window.YanceRuntimeErrors?.runtimeDiagnosticEvidence?.(value,{kind,module:kind==='error'?'运行时':'异步任务',filename:event.filename||'',line:event.lineno||0,column:event.colno||0})||{schemaVersion:1,at:new Date().toISOString(),module:kind==='error'?'运行时':'异步任务',kind,reasonCode:runtimeErrorCode(value)||'RENDERER_RUNTIME_FAILURE',severity:'error',fatal:true,recoverable:false,silent:false,detail:userFacingRuntimeError(value,'运行时异常')};r32RuntimeState.moduleErrors.push(evidence);r32RuntimeState.moduleErrors=r32RuntimeState.moduleErrors.slice(-20);r32RuntimeWrite();return evidence}
  function recordRuntimeProcessHealth(payload={}){const row={schemaVersion:1,at:String(payload.occurredAt||new Date().toISOString()),reasonCode:String(payload.reasonCode||'RUNTIME_PROCESS_HEALTH_EVENT'),scope:String(payload.scope||'child-process'),recoverable:payload.recoverable===true,fatal:payload.fatal===true,recoveryAction:String(payload.recoveryAction||''),processType:String(payload.processType||''),serviceName:String(payload.serviceName||''),reason:String(payload.reason||''),exitCode:Number(payload.exitCode||0)};r32RuntimeState.runtimeProcessHealth.push(row);r32RuntimeState.runtimeProcessHealth=r32RuntimeState.runtimeProcessHealth.slice(-20);r32RuntimeWrite();return row}
  window.addEventListener('error',e=>{const classification=classifyRuntimeFailure(e.error||e.message,{kind:'error'}),evidence=recordRendererRuntimeFailure('error',e.error||new Error(e.message||'运行时异常'),e);if(classification.silent)return;if(classification.recoverable){showSystemStatus('warning',classification.userMessage,'诊断',()=>runDiagnostics(),3200);return}showSystemStatus('error',`${classification.userMessage}（${evidence.reasonCode}）`,'诊断',()=>runDiagnostics(),0)});
  window.addEventListener('unhandledrejection',e=>{const classification=classifyRuntimeFailure(e.reason,{kind:'unhandledrejection'}),evidence=recordRendererRuntimeFailure('unhandledrejection',e.reason||new Error('异步任务失败'));if(classification.silent)return;if(classification.recoverable){showSystemStatus('warning',classification.userMessage,'诊断',()=>runDiagnostics(),3200);return}showSystemStatus('error',`${classification.userMessage}（${evidence.reasonCode}）`,'诊断',()=>runDiagnostics(),0)});
  window.yanceDesktop?.onRuntimeHealth?.(payload=>{const row=recordRuntimeProcessHealth(payload);window.dispatchEvent(new CustomEvent('yance:runtime-health',{detail:payload}));if(row.recoverable&&row.scope==='network-service'){showSystemStatus('warning','网络服务已重启，正在恢复联网数据','诊断',()=>runDiagnostics(),4200);return}if(row.fatal)showSystemStatus('error',`运行进程异常（${row.reasonCode}）`,'诊断',()=>runDiagnostics(),0)});
...
function diagnosticSnapshot(tests=[],error=null){const rows=Array.isArray(tests)?tests.map(row=>({name:diagnosticSafeText(row.name,160),status:['pass','warn','fail','skipped'].includes(row.status)?row.status:'fail',detail:diagnosticSafeText(row.detail,1200)})):[],summary={pass:rows.filter(row=>row.status==='pass').length,warn:rows.filter(row=>row.status==='warn').length,fail:rows.filter(row=>row.status==='fail').length,skipped:rows.filter(row=>row.status==='skipped').length};return {schemaVersion:1,generatedAt:new Date().toISOString(),privacyMode:true,status:error?'failed':summary.fail?'attention':(summary.warn||summary.skipped)?'attention':'pass',summary,workspace:{view:currentView,renderer:rendererEnvironmentSnapshot()},runtimeErrors:(r32RuntimeState.moduleErrors||[]).slice(-20).map(row=>({schemaVersion:Number(row.schemaVersion||1),at:diagnosticSafeText(row.at||new Date(Number(row.time||0)).toISOString(),80),module:diagnosticSafeText(row.module,120),kind:diagnosticSafeText(row.kind,80),reasonCode:diagnosticSafeText(row.reasonCode||'RENDERER_RUNTIME_FAILURE',160),severity:diagnosticSafeText(row.severity||'error',40),fatal:row.fatal===true,recoverable:row.recoverable===true,detail:diagnosticSafeText(row.detail,1200),filename:diagnosticSafeText(row.filename,300),line:Number(row.line||0),column:Number(row.column||0),stack:diagnosticSafeText(row.stack,3000)})),runtimeProcessHealth:(r32RuntimeState.runtimeProcessHealth||[]).slice(-20).map(row=>({at:diagnosticSafeText(row.at,80),reasonCode:diagnosticSafeText(row.reasonCode,160),scope:diagnosticSafeText(row.scope,120),recoverable:row.recoverable===true,fatal:row.fatal===true,recoveryAction:diagnosticSafeText(row.recoveryAction,160),processType:diagnosticSafeText(row.processType,80),serviceName:diagnosticSafeText(row.serviceName,240),reason:diagnosticSafeText(row.reason,100),exitCode:Number(row.exitCode||0)})),modelRuntime:window.__YanceModelRuntimeDiagnostics?{schemaVersion:Number(window.__YanceModelRuntimeDiagnostics.schemaVersion||1),status:diagnosticSafeText(window.__YanceModelRuntimeDiagnostics.status,40),attempt:Number(window.__YanceModelRuntimeDiagnostics.attempt||0),reasonCode:diagnosticSafeText(window.__YanceModelRuntimeDiagnostics.reasonCode,160),lastError:diagnosticSafeText(window.__YanceModelRuntimeDiagnostics.lastError,1200),lastUpdatedAt:diagnosticSafeText(window.__YanceModelRuntimeDiagnostics.lastUpdatedAt,80),nextRetryAt:Number(window.__YanceModelRuntimeDiagnostics.nextRetryAt||0),modelCount:Number(window.__YanceModelRuntimeDiagnostics.modelCount||0),routeCount:Number(window.__YanceModelRuntimeDiagnostics.routeCount||0),contactIndependent:window.__YanceModelRuntimeDiagnostics.contactIndependent===true}:null,tests:rows,...(error?{error:{reasonCode:diagnosticSafeText(runtimeErrorCode(error)||'WORKSPACE_DIAGNOSTICS_FAILED',120),message:diagnosticSafeText(userFacingRuntimeError(error,'系统诊断执行失败'),600)}}:{})}}
...
function runDiagnostics(rerun=false){const dlg=$('diagnosticDialog'),exportButton=$('exportDiagnostic'),authority=window.YanceDiagnosticSummaryAuthority;if(!dlg.open)dlg.showModal();if(exportButton)exportButton.disabled=true;$('diagnosticSummary').innerHTML='<article><b>…</b><span>正在检测</span></article><article><b>…</b><span>需关注</span></article><article><b>…</b><span>失败</span></article><article><b>…</b><span>未执行</span></article>';$('diagnosticList').innerHTML='<div class="diagnostic-row warn"><i>⟳</i><div><b>正在执行工作区检查与真实系统探针</b><p>结果将合并后端存储、AI、账号、备份、安全和运行时探针；工作区全绿不能覆盖系统失败。</p></div><em>请稍候</em></div>';return new Promise(resolve=>setTimeout(async()=>{try{const workspaceTests=await regressionChecks();let backendDiagnostics={tests:[]};try{const payload=await apiJson('/api/r32/system/diagnostics',{timeoutMs:60000});backendDiagnostics=payload.diagnostics||{tests:[]}}catch(error){backendDiagnostics={tests:[{id:'backend-diagnostics-fetch',name:'后端真实诊断读取',status:'fail',detail:userFacingRuntimeError(error,'无法读取后端真实诊断'),reasonCode:runtimeErrorCode(error)||'BACKEND_DIAGNOSTICS_UNAVAILABLE'}]}}const merged=authority?.merge?authority.merge(workspaceTests,backendDiagnostics):{rows:workspaceTests,summary:{pass:workspaceTests.filter(x=>x.status==='pass').length,warn:workspaceTests.filter(x=>x.status==='warn').length,fail:workspaceTests.filter(x=>x.status==='fail').length,skipped:0},overallStatus:workspaceTests.some(x=>x.status==='fail')?'fail':'pass'};const {pass,warn,fail,skipped}=merged.summary;lastDiagnosticSnapshot={...diagnosticSnapshot(merged.rows),authority:merged.authority||'YanceDiagnosticSummaryAuthority',overallStatus:merged.overallStatus,backendDiagnostics:{at:backendDiagnostics.at||'',pass:Number(backendDiagnostics.pass||0),warning:Number(backendDiagnostics.warning||0),fail:Number(backendDiagnostics.fail||0),skipped:Number(backendDiagnostics.skipped||0)}};$('diagnosticSummary').innerHTML=`<article><b>${htmlText(pass)}</b><span>通过</span></article><article><b>${htmlText(warn)}</b><span>需关注</span></article><article><b>${htmlText(fail)}</b><span>失败</span></article><article><b>${htmlText(skipped)}</b><span>未执行</span></article>`;$('diagnosticList').innerHTML=merged.rows.map(x=>`<div class="diagnostic-row ${htmlAttr(x.status)}"><i>${htmlText(x.status==='pass'?'✓':x.status==='fail'?'×':x.status==='skipped'?'–':'!')}</i><div><b>${htmlText(x.name)}</b><p>${htmlText(x.detail)}</p></div><em>${htmlText(x.status==='pass'?'通过':x.status==='fail'?'失败':x.status==='skipped'?'未执行':'关注')}</em></div>`).join('');const message=authority?.completionMessage?authority.completionMessage(merged):(fail?'工作区与系统诊断发现失败项':'工作区与系统诊断完成');if(fail)showSystemStatus('error',message,'查看',()=>dlg.showModal());else if(warn||skipped)showSystemStatus('warning',message,'查看',()=>dlg.showModal(),3200);else showSystemStatus('success',message,'',null,2200);resolve(lastDiagnosticSnapshot)}catch(error){const message=userFacingRuntimeError(error,'工作区与系统诊断执行失败');lastDiagnosticSnapshot=diagnosticSnapshot([],error);$('diagnosticList').innerHTML=`<div class="diagnostic-row fail"><i>×</i><div><b>工作区与系统诊断执行失败</b><p>${htmlText(message)}</p></div><em>失败</em></div>`;showSystemStatus('error',message,'重试',()=>runDiagnostics(true));resolve(lastDiagnosticSnapshot)}finally{if(exportButton)exportButton.disabled=false}},260))}
