...
def valid_daily_local_date(local_date: Any) -> str:
    ...
    if not DAILY_LOCAL_DATE_PATTERN.fullmatch(value):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail={"reasonCode": "YANCE_PARLANT_DAILY_LOCAL_DATE_INVALID", "message": "localDate must be a real calendar date"}) from exc
    return value
...
async def goal_projection(key: str, session=None) -> dict[str, Any]:
    graph = await find_goal(key)
    if graph is None:
        return {"ok": True, "available": True, "exists": False, "goalText": "", "progress": {"path": [], "completed": False}}
...
async def daily_goal_projection(key: str, local_date: str, session=None) -> dict[str, Any]:
    graph = await find_daily_goal(key, local_date)
    if graph is None:
        return {"ok": True, "available": True, "exists": False, "goalText": "", "progress": {"path": [], "completed": False}, "localDate": local_date}
    _ids, _agent, _customer, resolved_session = await ensure_relationship(key)
    session = session or resolved_session
    persisted_session = await RUNTIME.session_store.read_session(session.id)
    journey_id = graph.journey.id
